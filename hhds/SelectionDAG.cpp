...
static cl::opt<bool> EnableMemCpyDAGOpt("enable-memcpy-dag-opt", cl::Hidden, cl::init(true),
                                        cl::desc("Gang up loads and stores generated by inlining of memcpy"));

static cl::opt<int> MaxLdStGlue("ldstmemcpy-glue-max", cl::desc("Number limit for gluing ld/st of memcpy."), cl::Hidden,
                                cl::init(0));

static cl::opt<unsigned> MaxSteps("has-predecessor-max-steps", cl::Hidden, cl::init(8192),
                                  cl::desc("DAG combiner limit number of steps when searching DAG "
                                           "for predecessor nodes"));

static void NewSDValueDbgMsg(SDValue V, StringRef Msg, SelectionDAG* G) { LLVM_DEBUG(dbgs() << Msg; V.getNode()->dump(G);); }
...
bool ConstantFPSDNode::isExactlyValue(const APFloat& V) const { return getValueAPF().bitwiseIsEqual(V); }
...
bool ConstantFPSDNode::isValueValidForType(EVT VT, const APFloat& Val) {
  ...
  bool    losesInfo;
  (void)Val2.convert(VT.getFltSemantics(), APFloat::rmNearestTiesToEven, &losesInfo);
  return !losesInfo;
}
...
bool ISD::isConstantSplatVector(const SDNode* N, APInt& SplatVal) {
  ...
  return BV->isConstantSplat(SplatVal, SplatUndef, SplatBitSize, HasUndefs, EltSize, IsBigEndian) && EltSize == SplatBitSize;
}
...
bool ISD::isBuildVectorAllOnes(const SDNode* N) { return isConstantSplatVectorAllOnes(N, /*BuildVectorOnly*/ true); }

bool ISD::isBuildVectorAllZeros(const SDNode* N) { return isConstantSplatVectorAllZeros(N, /*BuildVectorOnly*/ true); }
...
bool ISD::isVectorShrinkable(const SDNode* N, unsigned NewEltSize, bool Signed) {
  ...
  if (N->getOpcode() == ISD::ZERO_EXTEND) {
    return (N->getOperand(0).getValueType().getScalarSizeInBits() <= NewEltSize) && !Signed;
  }
...
bool ISD::isFreezeUndef(const SDNode* N) { return N->getOpcode() == ISD::FREEZE && N->getOperand(0).isUndef(); }

template <typename ConstNodeType>
bool ISD::matchUnaryPredicateImpl(SDValue Op, std::function<bool(ConstNodeType*)> Match, bool AllowUndefs, bool AllowTruncation) {
  ...
  if (ISD::BUILD_VECTOR != Op.getOpcode() && ISD::SPLAT_VECTOR != Op.getOpcode()) {
    return false;
  }
...
template bool ISD::matchUnaryPredicateImpl<ConstantSDNode>(SDValue, std::function<bool(ConstantSDNode*)>, bool, bool);
template bool ISD::matchUnaryPredicateImpl<ConstantFPSDNode>(SDValue, std::function<bool(ConstantFPSDNode*)>, bool, bool);

bool ISD::matchBinaryPredicate(SDValue LHS, SDValue RHS, std::function<bool(ConstantSDNode*, ConstantSDNode*)> Match,
  ...
  if (LHS.getOpcode() != RHS.getOpcode() || (LHS.getOpcode() != ISD::BUILD_VECTOR && LHS.getOpcode() != ISD::SPLAT_VECTOR)) {
    return false;
  }
  ...
  for (unsigned i = 0, e = LHS.getNumOperands(); i != e; ++i) {
    ...
    if (!AllowTypeMismatch && (LHSOp.getValueType() != SVT || LHSOp.getValueType() != RHSOp.getValueType())) {
      return false;
    }
...
ISD::NodeType ISD::getInverseMinMaxOpcode(unsigned MinMaxOpc) {
  switch (MinMaxOpc) {
    default: llvm_unreachable("unrecognized opcode");
    case ISD::UMIN: return ISD::UMAX;
    case ISD::UMAX: return ISD::UMIN;
    case ISD::SMIN: return ISD::SMAX;
    case ISD::SMAX: return ISD::SMIN;
  }
}
...
ISD::NodeType ISD::getVecReduceBaseOpcode(unsigned VecReduceOpcode) {
  switch (VecReduceOpcode) {
    ...
    case ISD::VP_REDUCE_SEQ_FADD: return ISD::FADD;
    case ISD::VECREDUCE_FMUL:
    case ISD::VECREDUCE_SEQ_FMUL:
    case ISD::VP_REDUCE_FMUL:
    case ISD::VP_REDUCE_SEQ_FMUL: return ISD::FMUL;
    case ISD::VECREDUCE_ADD:
    case ISD::VP_REDUCE_ADD: return ISD::ADD;
    case ISD::VECREDUCE_MUL:
    case ISD::VP_REDUCE_MUL: return ISD::MUL;
    case ISD::VECREDUCE_AND:
    case ISD::VP_REDUCE_AND: return ISD::AND;
    case ISD::VECREDUCE_OR:
    case ISD::VP_REDUCE_OR: return ISD::OR;
    case ISD::VECREDUCE_XOR:
    case ISD::VP_REDUCE_XOR: return ISD::XOR;
    case ISD::VECREDUCE_SMAX:
    case ISD::VP_REDUCE_SMAX: return ISD::SMAX;
    case ISD::VECREDUCE_SMIN:
    case ISD::VP_REDUCE_SMIN: return ISD::SMIN;
    case ISD::VECREDUCE_UMAX:
    case ISD::VP_REDUCE_UMAX: return ISD::UMAX;
    case ISD::VECREDUCE_UMIN:
    case ISD::VP_REDUCE_UMIN: return ISD::UMIN;
    case ISD::VECREDUCE_FMAX:
    case ISD::VP_REDUCE_FMAX: return ISD::FMAXNUM;
    case ISD::VECREDUCE_FMIN:
    case ISD::VP_REDUCE_FMIN: return ISD::FMINNUM;
    case ISD::VECREDUCE_FMAXIMUM:
    case ISD::VP_REDUCE_FMAXIMUM: return ISD::FMAXIMUM;
    case ISD::VECREDUCE_FMINIMUM:
    case ISD::VP_REDUCE_FMINIMUM: return ISD::FMINIMUM;
  }
}
...
bool ISD::isVPReduction(unsigned Opcode) {
  switch (Opcode) {
    ...
    case ISD::VP_REDUCE_SEQ_FMUL: return true;
  }
}
...
#define BEGIN_REGISTER_VP_SDNODE(VPSD, LEGALPOS, TDNAME, MASKPOS, EVLPOS) \
  case ISD::VPSD: return EVLPOS;
#include "llvm/IR/VPIntrinsics.def"
  }
}

std::optional<unsigned> ISD::getBaseOpcodeForVP(unsigned VPOpcode, bool hasFPExcept) {
...
ISD::NodeType ISD::getExtForLoadExtType(bool IsFP, ISD::LoadExtType ExtType) {
  switch (ExtType) {
    case ISD::EXTLOAD: return IsFP ? ISD::FP_EXTEND : ISD::ANY_EXTEND;
    case ISD::SEXTLOAD: return ISD::SIGN_EXTEND;
    case ISD::ZEXTLOAD: return ISD::ZERO_EXTEND;
    default: break;
  }
...
ISD::CondCode ISD::getSetCCInverse(ISD::CondCode Op, EVT Type) { return getSetCCInverseImpl(Op, Type.isInteger()); }

ISD::CondCode ISD::GlobalISel::getSetCCInverse(ISD::CondCode Op, bool isIntegerLike) {
...
ISD::CondCode ISD::getSetCCOrOperation(ISD::CondCode Op1, ISD::CondCode Op2, EVT Type) {
...
ISD::CondCode ISD::getSetCCAndOperation(ISD::CondCode Op1, ISD::CondCode Op2, EVT Type) {
  bool IsInteger = Type.isInteger();
  if (IsInteger && (isSignedOp(Op1) | isSignedOp(Op2)) == 3) {
...
static void AddNodeIDOpcode(FoldingSetNodeID& ID, unsigned OpC) { ID.AddInteger(OpC); }

/// AddNodeIDValueTypes - Value type lists are intern'd so we can represent them
/// solely with their pointer.
static void AddNodeIDValueTypes(FoldingSetNodeID& ID, SDVTList VTList) { ID.AddPointer(VTList.VTs); }
...
static void AddNodeIDCustom(FoldingSetNodeID& ID, const SDNode* N) {
  switch (N->getOpcode()) {
    ...
    case ISD::MCSymbol: llvm_unreachable("Should only be used on nodes with operands");
    ...
    case ISD::ConstantFP: ID.AddPointer(cast<ConstantFPSDNode>(N)->getConstantFPValue()); break;
    ...
    case ISD::BasicBlock: ID.AddPointer(cast<BasicBlockSDNode>(N)->getBasicBlock()); break;
    case ISD::Register: ID.AddInteger(cast<RegisterSDNode>(N)->getReg().id()); break;
    case ISD::RegisterMask: ID.AddPointer(cast<RegisterMaskSDNode>(N)->getRegMask()); break;
    case ISD::SRCVALUE: ID.AddPointer(cast<SrcValueSDNode>(N)->getValue()); break;
    case ISD::FrameIndex:
    case ISD::TargetFrameIndex: ID.AddInteger(cast<FrameIndexSDNode>(N)->getIndex()); break;
    ...
    case ISD::AssertAlign: ID.AddInteger(cast<AssertAlignSDNode>(N)->getAlign().value()); break;
    case ISD::PREFETCH:
    case ISD::INTRINSIC_VOID:
    case ISD::INTRINSIC_W_CHAIN:
      // Handled by MemIntrinsicSDNode check after the switch.
      break;
    case ISD::MDNODE_SDNODE: ID.AddPointer(cast<MDNodeSDNode>(N)->getMD()); break;
...
static bool doNotCSE(SDNode* N) {
  ...
  switch (N->getOpcode()) {
    default: break;
    case ISD::HANDLENODE:
    case ISD::EH_LABEL: return true;  // Never CSE these nodes.
  }
...
void SelectionDAG::DeleteNodeNotInCSEMaps(SDNode* N) {
  assert(N->getIterator() != AllNodes.begin() && "Cannot delete the entry node!");
...
void SelectionDAG::verifyNode(SDNode* N) const {
  switch (N->getOpcode()) {
    ...
    case ISD::BUILD_PAIR: {
      ...
      assert(!VT.isVector() && (VT.isInteger() || VT.isFloatingPoint()) && "Wrong return type!");
      assert(N->getNumOperands() == 2 && "Wrong number of operands!");
      assert(N->getOperand(0).getValueType() == N->getOperand(1).getValueType() && "Mismatched operand types!");
      assert(N->getOperand(0).getValueType().isInteger() == VT.isInteger() && "Wrong operand type!");
      assert(VT.getSizeInBits() == 2 * N->getOperand(0).getValueSizeInBits() && "Wrong return type size");
      break;
    }
    case ISD::BUILD_VECTOR: {
      assert(N->getNumValues() == 1 && "Too many results!");
      assert(N->getValueType(0).isVector() && "Wrong return type!");
      assert(N->getNumOperands() == N->getValueType(0).getVectorNumElements() && "Wrong number of operands!");
      EVT EltVT = N->getValueType(0).getVectorElementType();
      for (const SDUse& Op : N->ops()) {
        assert(
            (Op.getValueType() == EltVT || (EltVT.isInteger() && Op.getValueType().isInteger() && EltVT.bitsLE(Op.getValueType())))
            && "Wrong operand type!");
        assert(Op.getValueType() == N->getOperand(0).getValueType() && "Operands must all have the same type");
      }
      break;
    }
  }
...
bool SelectionDAG::RemoveNodeFromCSEMaps(SDNode* N) {
  ...
  switch (N->getOpcode()) {
    ...
    case ISD::CONDCODE:
      assert(CondCodeNodes[cast<CondCodeSDNode>(N)->get()] && "Cond code doesn't exist!");
      Erased                                        = CondCodeNodes[cast<CondCodeSDNode>(N)->get()] != nullptr;
      CondCodeNodes[cast<CondCodeSDNode>(N)->get()] = nullptr;
      break;
    case ISD::ExternalSymbol: Erased = ExternalSymbols.erase(cast<ExternalSymbolSDNode>(N)->getSymbol()); break;
    case ISD::TargetExternalSymbol: {
      ExternalSymbolSDNode* ESN = cast<ExternalSymbolSDNode>(N);
      Erased = TargetExternalSymbols.erase(std::pair<std::string, unsigned>(ESN->getSymbol(), ESN->getTargetFlags()));
      break;
    }
    case ISD::MCSymbol: {
  ...
  // not subject to CSE.
  if (!Erased && N->getValueType(N->getNumValues() - 1) != MVT::Glue && !N->isMachineOpcode() && !doNotCSE(N)) {
    N->dump(this);
    dbgs() << "\n";
...
SDNode* SelectionDAG::FindModifiedNodeSlot(SDNode* N, SDValue Op, void*& InsertPos) {
  if (doNotCSE(N)) {
    return nullptr;
  }
...
/// return null, otherwise return a pointer to the slot it would take.  If a
/// node already exists with these operands, the slot will be non-null.
SDNode* SelectionDAG::FindModifiedNodeSlot(SDNode* N, SDValue Op1, SDValue Op2, void*& InsertPos) {
  if (doNotCSE(N)) {
    return nullptr;
  }
...
/// return null, otherwise return a pointer to the slot it would take.  If a
/// node already exists with these operands, the slot will be non-null.
SDNode* SelectionDAG::FindModifiedNodeSlot(SDNode* N, ArrayRef<SDValue> Ops, void*& InsertPos) {
  if (doNotCSE(N)) {
    return nullptr;
  }
...
Align SelectionDAG::getEVTAlign(EVT VT) const {
  Type* Ty = VT == MVT::iPTR ? PointerType::get(*getContext(), 0) : VT.getTypeForEVT(*getContext());
...
SelectionDAG::SelectionDAG(const TargetMachine& tm, CodeGenOptLevel OL)
    : TM(tm), OptLevel(OL), EntryNode(ISD::EntryToken, 0, DebugLoc(), getVTList(MVT::Other, MVT::Glue)), Root(getEntryNode()) {
  InsertNode(&EntryNode);
  DbgInfo = new SDDbgInfo();
}

void SelectionDAG::init(MachineFunction& NewMF, OptimizationRemarkEmitter& NewORE, Pass* PassPtr,
                        const TargetLibraryInfo* LibraryInfo, UniformityInfo* NewUA, ProfileSummaryInfo* PSIin,
                        BlockFrequencyInfo* BFIin, MachineModuleInfo& MMIin, FunctionVarLocs const* VarLocs) {
  MF           = &NewMF;
...
bool SelectionDAG::shouldOptForSize() const { return llvm::shouldOptimizeForSize(FLI->MBB->getBasicBlock(), PSI, BFI); }
...
SDNode* SelectionDAG::FindNodeOrInsertPos(const FoldingSetNodeID& ID, void*& InsertPos) {
  ...
  if (N) {
    switch (N->getOpcode()) {
      ...
      case ISD::ConstantFP:
        llvm_unreachable(
            "Querying for Constant and ConstantFP nodes requires "
...
SDNode* SelectionDAG::FindNodeOrInsertPos(const FoldingSetNodeID& ID, const SDLoc& DL, void*& InsertPos) {
  SDNode* N = CSEMap.FindNodeOrInsertPos(ID, InsertPos);
  if (N) {
    switch (N->getOpcode()) {
      case ISD::Constant:
...
SDValue SelectionDAG::getFPExtendOrRound(SDValue Op, const SDLoc& DL, EVT VT) {
  return VT.bitsGT(Op.getValueType()) ? getNode(ISD::FP_EXTEND, DL, VT, Op)
                                      : getNode(ISD::FP_ROUND, DL, VT, Op, getIntPtrConstant(0, DL, /*isTarget=*/true));
}

std::pair<SDValue, SDValue> SelectionDAG::getStrictFPExtendOrRound(SDValue Op, SDValue Chain, const SDLoc& DL, EVT VT) {
  assert(!VT.bitsEq(Op.getValueType()) && "Strict no-op FP extend/round not allowed.");
  SDValue Res = VT.bitsGT(Op.getValueType())
                    ? getNode(ISD::STRICT_FP_EXTEND, DL, {VT, MVT::Other}, {Chain, Op})
                    : getNode(ISD::STRICT_FP_ROUND, DL, {VT, MVT::Other}, {Chain, Op, getIntPtrConstant(0, DL, /*isTarget=*/true)});
...
SDValue SelectionDAG::getAnyExtOrTrunc(SDValue Op, const SDLoc& DL, EVT VT) {
  return VT.bitsGT(Op.getValueType()) ? getNode(ISD::ANY_EXTEND, DL, VT, Op) : getNode(ISD::TRUNCATE, DL, VT, Op);
}

SDValue SelectionDAG::getSExtOrTrunc(SDValue Op, const SDLoc& DL, EVT VT) {
  return VT.bitsGT(Op.getValueType()) ? getNode(ISD::SIGN_EXTEND, DL, VT, Op) : getNode(ISD::TRUNCATE, DL, VT, Op);
}

SDValue SelectionDAG::getZExtOrTrunc(SDValue Op, const SDLoc& DL, EVT VT) {
  return VT.bitsGT(Op.getValueType()) ? getNode(ISD::ZERO_EXTEND, DL, VT, Op) : getNode(ISD::TRUNCATE, DL, VT, Op);
}

SDValue SelectionDAG::getBitcastedAnyExtOrTrunc(SDValue Op, const SDLoc& DL, EVT VT) {
  assert(!VT.isVector());
...
SDValue SelectionDAG::getBitcastedSExtOrTrunc(SDValue Op, const SDLoc& DL, EVT VT) {
  assert(!VT.isVector());
  auto    Type = Op.getValueType();
  SDValue DestOp;
  if (Type == VT) {
...
SDValue SelectionDAG::getBitcastedZExtOrTrunc(SDValue Op, const SDLoc& DL, EVT VT) {
  assert(!VT.isVector());
  auto    Type = Op.getValueType();
  SDValue DestOp;
  if (Type == VT) {
...
SDValue SelectionDAG::getBoolExtOrTrunc(SDValue Op, const SDLoc& SL, EVT VT, EVT OpVT) {
...
SDValue SelectionDAG::getZeroExtendInReg(SDValue Op, const SDLoc& DL, EVT VT) {
  ...
  assert(VT.isInteger() && OpVT.isInteger() && "Cannot getZeroExtendInReg FP types");
  assert(VT.isVector() == OpVT.isVector() &&
         "getZeroExtendInReg type should be vector iff the operand "
         "type is vector!");
  assert((!VT.isVector() || VT.getVectorElementCount() == OpVT.getVectorElementCount())
         && "Vector element counts must match in getZeroExtendInReg");
  assert(VT.bitsLE(OpVT) && "Not extending!");
  if (OpVT == VT) {
    return Op;
  }
  APInt Imm = APInt::getLowBitsSet(OpVT.getScalarSizeInBits(), VT.getScalarSizeInBits());
  return getNode(ISD::AND, DL, OpVT, Op, getConstant(Imm, DL, OpVT));
}

SDValue SelectionDAG::getVPZeroExtendInReg(SDValue Op, SDValue Mask, SDValue EVL, const SDLoc& DL, EVT VT) {
  EVT OpVT = Op.getValueType();
  assert(VT.isInteger() && OpVT.isInteger() && "Cannot getVPZeroExtendInReg FP types");
  assert(VT.isVector() && OpVT.isVector() && "getVPZeroExtendInReg type and operand type should be vector!");
  assert(VT.getVectorElementCount() == OpVT.getVectorElementCount() && "Vector element counts must match in getZeroExtendInReg");
  assert(VT.bitsLE(OpVT) && "Not extending!");
  if (OpVT == VT) {
    return Op;
  }
  APInt Imm = APInt::getLowBitsSet(OpVT.getScalarSizeInBits(), VT.getScalarSizeInBits());
  return getNode(ISD::VP_AND, DL, OpVT, Op, getConstant(Imm, DL, OpVT), Mask, EVL);
}
...
SDValue SelectionDAG::getVPLogicalNOT(const SDLoc& DL, SDValue Val, SDValue Mask, SDValue EVL, EVT VT) {
  SDValue TrueValue = getBoolConstant(true, DL, VT, VT);
  return getNode(ISD::VP_XOR, DL, VT, Val, TrueValue, Mask, EVL);
}

SDValue SelectionDAG::getVPPtrExtOrTrunc(const SDLoc& DL, EVT VT, SDValue Op, SDValue Mask, SDValue EVL) {
  return getVPZExtOrTrunc(DL, VT, Op, Mask, EVL);
}

SDValue SelectionDAG::getVPZExtOrTrunc(const SDLoc& DL, EVT VT, SDValue Op, SDValue Mask, SDValue EVL) {
...
SDValue SelectionDAG::getBoolConstant(bool V, const SDLoc& DL, EVT VT, EVT OpVT) {
  if (!V) {
  ...
  switch (TLI->getBooleanContents(OpVT)) {
    ...
    case TargetLowering::UndefinedBooleanContent: return getConstant(1, DL, VT);
    case TargetLowering::ZeroOrNegativeOneBooleanContent: return getAllOnesConstant(DL, VT);
  }
  llvm_unreachable("Unexpected boolean content enum!");
}

SDValue SelectionDAG::getConstant(uint64_t Val, const SDLoc& DL, EVT VT, bool isT, bool isO) {
  return getConstant(APInt(VT.getScalarSizeInBits(), Val, /*isSigned=*/false), DL, VT, isT, isO);
}

SDValue SelectionDAG::getConstant(const APInt& Val, const SDLoc& DL, EVT VT, bool isT, bool isO) {
  return getConstant(*ConstantInt::get(*Context, Val), DL, VT, isT, isO);
}

SDValue SelectionDAG::getConstant(const ConstantInt& Val, const SDLoc& DL, EVT VT, bool isT, bool isO) {
  ...
  if (VT.isVector() && TLI->getTypeAction(*getContext(), EltVT) == TargetLowering::TypePromoteInteger) {
  ...
  // only legalize if the DAG tells us we must produce legal types.
           ...
           && TLI->getTypeAction(*getContext(), EltVT) == TargetLowering::TypeExpandInteger) {
    ...
    if (VT.isScalableVector() || TLI->isOperationLegal(ISD::SPLAT_VECTOR, VT)) {
      assert(EltVT.getSizeInBits() % ViaEltSizeInBits == 0 && "Can only handle an even split!");
      unsigned Parts = EltVT.getSizeInBits() / ViaEltSizeInBits;

      SmallVector<SDValue, 2> ScalarParts;
      for (unsigned i = 0; i != Parts; ++i) {
        ScalarParts.push_back(getConstant(NewVal.extractBits(ViaEltSizeInBits, i * ViaEltSizeInBits), DL, ViaEltVT, isT, isO));
      }
    ...
    for (unsigned i = 0; i < ViaVecNumElts / VT.getVectorNumElements(); ++i) {
      EltParts.push_back(getConstant(NewVal.extractBits(ViaEltSizeInBits, i * ViaEltSizeInBits), DL, ViaEltVT, isT, isO));
    }
  ...
  assert(Elt->getBitWidth() == EltVT.getSizeInBits() && "APInt size does not match type size!");
...
SDValue SelectionDAG::getSignedConstant(int64_t Val, const SDLoc& DL, EVT VT, bool isT, bool isO) {
  unsigned Size = VT.getScalarSizeInBits();
  return getConstant(APInt(Size, Val, /*isSigned=*/true), DL, VT, isT, isO);
}

SDValue SelectionDAG::getAllOnesConstant(const SDLoc& DL, EVT VT, bool IsTarget, bool IsOpaque) {
  return getConstant(APInt::getAllOnes(VT.getScalarSizeInBits()), DL, VT, IsTarget, IsOpaque);
}

SDValue SelectionDAG::getIntPtrConstant(uint64_t Val, const SDLoc& DL, bool isTarget) {
  return getConstant(Val, DL, TLI->getPointerTy(getDataLayout()), isTarget);
}

SDValue SelectionDAG::getShiftAmountConstant(uint64_t Val, EVT VT, const SDLoc& DL) {
...
SDValue SelectionDAG::getShiftAmountConstant(const APInt& Val, EVT VT, const SDLoc& DL) {
  assert(Val.ult(VT.getScalarSizeInBits()) && "Out of range shift");
  return getShiftAmountConstant(Val.getZExtValue(), VT, DL);
}

SDValue SelectionDAG::getVectorIdxConstant(uint64_t Val, const SDLoc& DL, bool isTarget) {
  return getConstant(Val, DL, TLI->getVectorIdxTy(getDataLayout()), isTarget);
}

SDValue SelectionDAG::getConstantFP(const APFloat& V, const SDLoc& DL, EVT VT, bool isTarget) {
  return getConstantFP(*ConstantFP::get(*getContext(), V), DL, VT, isTarget);
}

SDValue SelectionDAG::getConstantFP(const ConstantFP& V, const SDLoc& DL, EVT VT, bool isTarget) {
...
SDValue SelectionDAG::getConstantFP(double Val, const SDLoc& DL, EVT VT, bool isTarget) {
  EVT EltVT = VT.getScalarType();
  if (EltVT == MVT::f32) {
  ...
  if (EltVT == MVT::f80 || EltVT == MVT::f128 || EltVT == MVT::ppcf128 || EltVT == MVT::f16 || EltVT == MVT::bf16) {
    bool    Ignored;
    APFloat APF = APFloat(Val);
    APF.convert(EltVT.getFltSemantics(), APFloat::rmNearestTiesToEven, &Ignored);
    return getConstantFP(APF, DL, VT, isTarget);
  }
  llvm_unreachable("Unsupported type in getConstantFP");
}

SDValue SelectionDAG::getGlobalAddress(const GlobalValue* GV, const SDLoc& DL, EVT VT, int64_t Offset, bool isTargetGA,
                                       unsigned TargetFlags) {
  assert((TargetFlags == 0 || isTargetGA) && "Cannot set target flags on target-independent globals");
  ...
  auto* N = newSDNode<GlobalAddressSDNode>(Opc, DL.getIROrder(), DL.getDebugLoc(), GV, VTs, Offset, TargetFlags);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
  return SDValue(N, 0);
}
...
SDValue SelectionDAG::getJumpTable(int JTI, EVT VT, bool isTarget, unsigned TargetFlags) {
  assert((TargetFlags == 0 || isTarget) && "Cannot set target flags on target-independent jump tables");
...
SDValue SelectionDAG::getJumpTableDebugInfo(int JTI, SDValue Chain, const SDLoc& DL) {
  EVT PTy = getTargetLoweringInfo().getPointerTy(getDataLayout());
  return getNode(ISD::JUMP_TABLE_DEBUG_INFO, DL, MVT::Glue, Chain, getTargetConstant(static_cast<uint64_t>(JTI), DL, PTy, true));
}

SDValue SelectionDAG::getConstantPool(const Constant* C, EVT VT, MaybeAlign Alignment, int Offset, bool isTarget,
                                      unsigned TargetFlags) {
  assert((TargetFlags == 0 || isTarget) && "Cannot set target flags on target-independent globals");
  if (!Alignment) {
    Alignment = shouldOptForSize() ? getDataLayout().getABITypeAlign(C->getType()) : getDataLayout().getPrefTypeAlign(C->getType());
  }
  ...
  auto* N = newSDNode<ConstantPoolSDNode>(isTarget, C, VTs, Offset, *Alignment, TargetFlags);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
...
SDValue SelectionDAG::getConstantPool(MachineConstantPoolValue* C, EVT VT, MaybeAlign Alignment, int Offset, bool isTarget,
                                      unsigned TargetFlags) {
  assert((TargetFlags == 0 || isTarget) && "Cannot set target flags on target-independent globals");
  if (!Alignment) {
...
SDValue SelectionDAG::getValueType(EVT VT) {
  if (VT.isSimple() && (unsigned)VT.getSimpleVT().SimpleTy >= ValueTypeNodes.size()) {
    ValueTypeNodes.resize(VT.getSimpleVT().SimpleTy + 1);
  }

  SDNode*& N = VT.isExtended() ? ExtendedValueTypeNodes[VT] : ValueTypeNodes[VT.getSimpleVT().SimpleTy];
...
SDValue SelectionDAG::getTargetExternalSymbol(const char* Sym, EVT VT, unsigned TargetFlags) {
  SDNode*& N = TargetExternalSymbols[std::pair<std::string, unsigned>(Sym, TargetFlags)];
  if (N) {
    return SDValue(N, 0);
  }
...
SDValue SelectionDAG::getVScale(const SDLoc& DL, EVT VT, APInt MulImm, bool ConstantFold) {
  assert(MulImm.getBitWidth() == VT.getSizeInBits() && "APInt size does not match type size!");
...
SDValue SelectionDAG::getElementCount(const SDLoc& DL, EVT VT, ElementCount EC, bool ConstantFold) {
  if (EC.isScalable()) {
...
SDValue SelectionDAG::getStepVector(const SDLoc& DL, EVT ResVT, const APInt& StepVal) {
  assert(ResVT.getScalarSizeInBits() == StepVal.getBitWidth());
  if (ResVT.isScalableVector()) {
    return getNode(ISD::STEP_VECTOR, DL, ResVT, getTargetConstant(StepVal, DL, ResVT.getVectorElementType()));
  }

  SmallVector<SDValue, 16> OpsStepConstants;
  for (uint64_t i = 0; i < ResVT.getVectorNumElements(); i++) {
    OpsStepConstants.push_back(getConstant(StepVal * i, DL, ResVT.getVectorElementType()));
  }
...
SDValue SelectionDAG::getVectorShuffle(EVT VT, const SDLoc& dl, SDValue N1, SDValue N2, ArrayRef<int> Mask) {
  assert(VT.getVectorNumElements() == Mask.size() && "Must have the same number of vector elements as mask elements!");
  assert(VT == N1.getValueType() && VT == N2.getValueType() && "Invalid VECTOR_SHUFFLE");
  ...
  int NElts = Mask.size();
  assert(llvm::all_of(Mask, [&](int M) { return M < (NElts * 2) && M >= -1; }) && "Index out of range");
  ...
  if (N2Undef) {
    ...
    if (auto* BV = dyn_cast<BuildVectorSDNode>(V)) {
      ...
      bool SameNumElts = V.getValueType().getVectorNumElements() == VT.getVectorNumElements();
  ...
  auto* N = newSDNode<ShuffleVectorSDNode>(VTs, dl.getIROrder(), dl.getDebugLoc(), MaskAlloc);
...
SDValue SelectionDAG::getEHLabel(const SDLoc& dl, SDValue Root, MCSymbol* Label) {
  return getLabelNode(ISD::EH_LABEL, dl, Root, Label);
}

SDValue SelectionDAG::getLabelNode(unsigned Opcode, const SDLoc& dl, SDValue Root, MCSymbol* Label) {
  FoldingSetNodeID ID;
  SDValue          Ops[] = {Root};
  ...
  auto* N = newSDNode<LabelSDNode>(Opcode, dl.getIROrder(), dl.getDebugLoc(), Label);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getBlockAddress(const BlockAddress* BA, EVT VT, int64_t Offset, bool isTarget, unsigned TargetFlags) {
...
SDValue SelectionDAG::getAddrSpaceCast(const SDLoc& dl, EVT VT, SDValue Ptr, unsigned SrcAS, unsigned DestAS) {
  SDVTList         VTs   = getVTList(VT);
  SDValue          Ops[] = {Ptr};
  FoldingSetNodeID ID;
  ...
  auto* N = newSDNode<AddrSpaceCastSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, SrcAS, DestAS);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getFreeze(SDValue V) { return getNode(ISD::FREEZE, SDLoc(V), V.getValueType(), V); }
...
static bool canFoldStoreIntoLibCallOutputPointers(StoreSDNode* StoreNode, SDNode* FPNode) {
  ...
  return !SDNode::hasPredecessorHelper(FPNode, Visited, DeferredNodes, MaxSteps);
}

bool SelectionDAG::expandMultipleResultFPLibCall(RTLIB::Libcall LC, SDNode* Node, SmallVectorImpl<SDValue>& Results,
  ...
  auto getVecDesc = [&]() -> VecDesc const* {
    for (bool Masked : {false, true}) {
      if (VecDesc const* VD = getLibInfo().getVectorMappingInfo(LCName, VT.getVectorElementCount(), Masked)) {
        return VD;
      }
    }
    return nullptr;
  ...
  for (SDNode* User : Node->users()) {
    ...
    if (ST->getAlign() < getDataLayout().getABITypeAlign(StoreType->getScalarType())) {
      continue;
    }
    // Avoid:
  ...
  Type*   RetType = CallRetResNo.has_value() ? Node->getValueType(*CallRetResNo).getTypeForEVT(Ctx) : Type::getVoidTy(Ctx);
  SDValue InChain = StoresInChain ? StoresInChain : getEntryNode();
  SDValue Callee  = getExternalSymbol(VD ? VD->getVectorFnName().data() : LCName, TLI->getPointerTy(getDataLayout()));
  TargetLowering::CallLoweringInfo CLI(*this);
  CLI.setDebugLoc(DL).setChain(InChain).setLibCallee(TLI->getLibcallCallingConv(LC), RetType, Callee, std::move(Args));
  ...
  for (auto [ResNo, ResultPtr] : llvm::enumerate(ResultPtrs)) {
    ...
    if (StoreSDNode* ST = ResultStores[ResNo]) {
      ...
      PtrInfo = MachinePointerInfo::getFixedStack(getMachineFunction(), cast<FrameIndexSDNode>(ResultPtr)->getIndex());
    }
    SDValue LoadResult = getLoad(Node->getValueType(ResNo), DL, CallChain, ResultPtr, PtrInfo);
...
SDValue SelectionDAG::expandVAArg(SDNode* Node) {
  ...
  SDValue VAListLoad = getLoad(TLI.getPointerTy(getDataLayout()), dl, Tmp1, Tmp2, MachinePointerInfo(V));
  SDValue VAList     = VAListLoad;

  if (MA && *MA > TLI.getMinStackArgumentAlignment()) {
    VAList = getNode(ISD::ADD, dl, VAList.getValueType(), VAList, getConstant(MA->value() - 1, dl, VAList.getValueType()));

    VAList
        = getNode(ISD::AND, dl, VAList.getValueType(), VAList, getSignedConstant(-(int64_t)MA->value(), dl, VAList.getValueType()));
  }

  // Increment the pointer, VAList, to the next vaarg
  Tmp1 = getNode(ISD::ADD,
                 dl,
                 VAList.getValueType(),
                 VAList,
                 getConstant(getDataLayout().getTypeAllocSize(VT.getTypeForEVT(*getContext())), dl, VAList.getValueType()));
  // Store the incremented VAList to the legalized pointer
  Tmp1 = getStore(VAListLoad.getValue(1), dl, Tmp1, Tmp2, MachinePointerInfo(V));
...
SDValue SelectionDAG::expandVACopy(SDNode* Node) {
  ...
  SDValue Tmp1 = getLoad(TLI.getPointerTy(getDataLayout()), dl, Node->getOperand(0), Node->getOperand(2), MachinePointerInfo(VS));
  return getStore(Tmp1.getValue(1), dl, Tmp1, Node->getOperand(1), MachinePointerInfo(VD));
}
...
Align SelectionDAG::getReducedAlign(EVT VT, bool UseABI) {
  ...
  if (RedAlign > StackAlign) {
    ...
    TLI->getVectorTypeBreakdown(*getContext(), VT, IntermediateVT, NumIntermediates, RegisterVT);
...
SDValue SelectionDAG::CreateStackTemporary(TypeSize Bytes, Align Alignment) {
  ...
  int FrameIdx = MFI.CreateStackObject(Bytes.getKnownMinValue(), Alignment, false, nullptr, StackID);
  return getFrameIndex(FrameIdx, TLI->getFrameIndexTy(getDataLayout()));
}

SDValue SelectionDAG::CreateStackTemporary(EVT VT, unsigned minAlign) {
  Type* Ty         = VT.getTypeForEVT(*getContext());
  Align StackAlign = std::max(getDataLayout().getPrefTypeAlign(Ty), Align(minAlign));
...
SDValue SelectionDAG::CreateStackTemporary(EVT VT1, EVT VT2) {
  ...
  assert(VT1Size.isScalable() == VT2Size.isScalable() &&
         ...
         "temporary");
  TypeSize Bytes = VT1Size.getKnownMinValue() > VT2Size.getKnownMinValue() ? VT1Size : VT2Size;
...
SDValue SelectionDAG::FoldSetCC(EVT VT, SDValue N1, SDValue N2, ISD::CondCode Cond, const SDLoc& dl) {
  EVT OpVT = N1.getValueType();

  auto GetUndefBooleanConstant = [&]() {
    if (VT.getScalarType() == MVT::i1 || TLI->getBooleanContents(OpVT) == TargetLowering::UndefinedBooleanContent) {
      return getUNDEF(VT);
    }
  ...
  switch (Cond) {
    ...
    case ISD::SETUO:
    case ISD::SETUEQ:
    case ISD::SETUNE: assert(!OpVT.isInteger() && "Illegal setcc for integer!"); break;
  }
  ...
  if (OpVT.isInteger()) {
    ...
    if ((N1.isUndef() || N2.isUndef()) && (Cond == ISD::SETEQ || Cond == ISD::SETNE)) {
  ...
  if (ConstantSDNode* N2C = dyn_cast<ConstantSDNode>(N2)) {
    ...
    if (ConstantSDNode* N1C = dyn_cast<ConstantSDNode>(N1)) {
      ...
      return getBoolConstant(ICmpInst::compare(C1, C2, getICmpCondCode(Cond)), dl, VT, OpVT);
    }
  }
  ...
  if (N1CFP && N2CFP) {
    ...
    switch (Cond) {
      ...
      case ISD::SETEQ:
        ...
        [[fallthrough]];
      case ISD::SETOEQ: return getBoolConstant(R == APFloat::cmpEqual, dl, VT, OpVT);
      case ISD::SETNE:
        if (R == APFloat::cmpUnordered) {
          return GetUndefBooleanConstant();
        }
        [[fallthrough]];
      case ISD::SETONE: return getBoolConstant(R == APFloat::cmpGreaterThan || R == APFloat::cmpLessThan, dl, VT, OpVT);
      case ISD::SETLT:
        if (R == APFloat::cmpUnordered) {
          return GetUndefBooleanConstant();
        }
        [[fallthrough]];
      case ISD::SETOLT: return getBoolConstant(R == APFloat::cmpLessThan, dl, VT, OpVT);
      case ISD::SETGT:
        if (R == APFloat::cmpUnordered) {
          return GetUndefBooleanConstant();
        }
        [[fallthrough]];
      case ISD::SETOGT: return getBoolConstant(R == APFloat::cmpGreaterThan, dl, VT, OpVT);
      case ISD::SETLE:
        if (R == APFloat::cmpUnordered) {
          return GetUndefBooleanConstant();
        }
        [[fallthrough]];
      case ISD::SETOLE: return getBoolConstant(R == APFloat::cmpLessThan || R == APFloat::cmpEqual, dl, VT, OpVT);
      case ISD::SETGE:
        if (R == APFloat::cmpUnordered) {
          return GetUndefBooleanConstant();
        }
        [[fallthrough]];
      case ISD::SETOGE: return getBoolConstant(R == APFloat::cmpGreaterThan || R == APFloat::cmpEqual, dl, VT, OpVT);
      case ISD::SETO: return getBoolConstant(R != APFloat::cmpUnordered, dl, VT, OpVT);
      case ISD::SETUO: return getBoolConstant(R == APFloat::cmpUnordered, dl, VT, OpVT);
      case ISD::SETUEQ: return getBoolConstant(R == APFloat::cmpUnordered || R == APFloat::cmpEqual, dl, VT, OpVT);
      case ISD::SETUNE: return getBoolConstant(R != APFloat::cmpEqual, dl, VT, OpVT);
      case ISD::SETULT: return getBoolConstant(R == APFloat::cmpUnordered || R == APFloat::cmpLessThan, dl, VT, OpVT);
      case ISD::SETUGT: return getBoolConstant(R == APFloat::cmpGreaterThan || R == APFloat::cmpUnordered, dl, VT, OpVT);
      case ISD::SETULE: return getBoolConstant(R != APFloat::cmpGreaterThan, dl, VT, OpVT);
      case ISD::SETUGE: return getBoolConstant(R != APFloat::cmpLessThan, dl, VT, OpVT);
    }
  ...
  } else if ((N2CFP && N2CFP->getValueAPF().isNaN()) || (OpVT.isFloatingPoint() && (N1.isUndef() || N2.isUndef()))) {
...
bool SelectionDAG::MaskedValueIsZero(SDValue V, const APInt& Mask, unsigned Depth) const {
...
bool SelectionDAG::MaskedValueIsZero(SDValue V, const APInt& Mask, const APInt& DemandedElts, unsigned Depth) const {
...
/// DemandedElts.  We use this predicate to simplify operations downstream.
bool SelectionDAG::MaskedVectorIsZero(SDValue V, const APInt& DemandedElts, unsigned Depth /* = 0 */) const {
...
APInt SelectionDAG::computeVectorKnownZeroElements(SDValue Op, const APInt& DemandedElts, unsigned Depth) const {
...
bool SelectionDAG::isSplatValue(SDValue V, const APInt& DemandedElts, APInt& UndefElts, unsigned Depth) const {
  unsigned Opcode = V.getOpcode();
  EVT      VT     = V.getValueType();
  assert(VT.isVector() && "Vector type expected");
  assert((!VT.isScalableVector() || DemandedElts.getBitWidth() == 1) && "scalable demanded bits are ignored");
  ...
  switch (Opcode) {
    case ISD::SPLAT_VECTOR:
      UndefElts = V.getOperand(0).isUndef() ? APInt::getAllOnes(DemandedElts.getBitWidth()) : APInt(DemandedElts.getBitWidth(), 0);
      return true;
    case ISD::ADD:
    case ISD::SUB:
    case ISD::AND:
    ...
    case ISD::OR: {
      ...
      if (isSplatValue(LHS, DemandedElts, UndefLHS, Depth + 1) && isSplatValue(RHS, DemandedElts, UndefRHS, Depth + 1)
    ...
    case ISD::ZERO_EXTEND: return isSplatValue(V.getOperand(0), DemandedElts, UndefElts, Depth + 1);
    default:
      if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN || Opcode == ISD::INTRINSIC_W_CHAIN
          || Opcode == ISD::INTRINSIC_VOID) {
        return TLI->isSplatValueForTargetNode(V, DemandedElts, UndefElts, *this, Depth);
      }
      break;
  }
  ...
  switch (Opcode) {
    ...
    case ISD::VECTOR_SHUFFLE: {
      ...
      if ((DemandedLHS.isZero() && DemandedRHS.isZero()) || (!DemandedLHS.isZero() && !DemandedRHS.isZero())) {
        return false;
      }
      ...
      auto CheckSplatSrc = [&](SDValue Src, const APInt& SrcElts) {
        APInt SrcUndefs;
        return (SrcElts.popcount() == 1) || (isSplatValue(Src, SrcElts, SrcUndefs, Depth + 1) && (SrcElts & SrcUndefs).isZero());
      };
...
bool SelectionDAG::isSplatValue(SDValue V, bool AllowUndefs) const {
  ...
  APInt DemandedElts = APInt::getAllOnes(VT.isScalableVector() ? 1 : VT.getVectorNumElements());
  return isSplatValue(V, DemandedElts, UndefElts) && (AllowUndefs || !UndefElts);
}
...
SDValue SelectionDAG::getSplatSourceVector(SDValue V, int& SplatIdx) {
  ...
  switch (Opcode) {
    ...
    case ISD::SPLAT_VECTOR: SplatIdx = 0; return V;
    case ISD::VECTOR_SHUFFLE: {
...
std::optional<ConstantRange> SelectionDAG::getValidShiftAmountRange(SDValue V, const APInt& DemandedElts, unsigned Depth) const {
  assert((V.getOpcode() == ISD::SHL || V.getOpcode() == ISD::SRL || V.getOpcode() == ISD::SRA) && "Unknown shift node");
  ...
  if (auto* BV = dyn_cast<BuildVectorSDNode>(V.getOperand(1))) {
    ...
    assert(((!MinAmt && !MaxAmt) || (MinAmt && MaxAmt)) && "Failed to find matching min/max shift amounts");
    if (MinAmt && MaxAmt) {
...
std::optional<uint64_t> SelectionDAG::getValidShiftAmount(SDValue V, const APInt& DemandedElts, unsigned Depth) const {
  assert((V.getOpcode() == ISD::SHL || V.getOpcode() == ISD::SRL || V.getOpcode() == ISD::SRA) && "Unknown shift node");
  if (std::optional<ConstantRange> AmtRange = getValidShiftAmountRange(V, DemandedElts, Depth)) {
...
std::optional<uint64_t> SelectionDAG::getValidShiftAmount(SDValue V, unsigned Depth) const {
  EVT   VT           = V.getValueType();
  APInt DemandedElts = VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorNumElements()) : APInt(1, 1);
  return getValidShiftAmount(V, DemandedElts, Depth);
}

std::optional<uint64_t> SelectionDAG::getValidMinimumShiftAmount(SDValue V, const APInt& DemandedElts, unsigned Depth) const {
  assert((V.getOpcode() == ISD::SHL || V.getOpcode() == ISD::SRL || V.getOpcode() == ISD::SRA) && "Unknown shift node");
  if (std::optional<ConstantRange> AmtRange = getValidShiftAmountRange(V, DemandedElts, Depth)) {
...
std::optional<uint64_t> SelectionDAG::getValidMinimumShiftAmount(SDValue V, unsigned Depth) const {
  EVT   VT           = V.getValueType();
  APInt DemandedElts = VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorNumElements()) : APInt(1, 1);
  return getValidMinimumShiftAmount(V, DemandedElts, Depth);
}

std::optional<uint64_t> SelectionDAG::getValidMaximumShiftAmount(SDValue V, const APInt& DemandedElts, unsigned Depth) const {
  assert((V.getOpcode() == ISD::SHL || V.getOpcode() == ISD::SRL || V.getOpcode() == ISD::SRA) && "Unknown shift node");
  if (std::optional<ConstantRange> AmtRange = getValidShiftAmountRange(V, DemandedElts, Depth)) {
...
std::optional<uint64_t> SelectionDAG::getValidMaximumShiftAmount(SDValue V, unsigned Depth) const {
  EVT   VT           = V.getValueType();
  APInt DemandedElts = VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorNumElements()) : APInt(1, 1);
...
KnownBits SelectionDAG::computeKnownBits(SDValue Op, const APInt& DemandedElts, unsigned Depth) const {
  unsigned BitWidth = Op.getScalarValueSizeInBits();
  ...
  assert((!Op.getValueType().isFixedLengthVector() || NumElts == Op.getValueType().getVectorNumElements())
  ...
  switch (Opcode) {
    case ISD::MERGE_VALUES: return computeKnownBits(Op.getOperand(Op.getResNo()), DemandedElts, Depth + 1);
    case ISD::SPLAT_VECTOR: {
      SDValue SrcOp = Op.getOperand(0);
      assert(SrcOp.getValueSizeInBits() >= BitWidth && "Expected SPLAT_VECTOR implicit truncation");
    ...
    case ISD::SPLAT_VECTOR_PARTS: {
      ...
      assert(ScalarSize * Op.getNumOperands() == BitWidth && "Expected SPLAT_VECTOR_PARTS scalars to cover element width");
    ...
    case ISD::STEP_VECTOR: {
      ...
      const APInt MinNumElts = APInt(BitWidth, Op.getValueType().getVectorMinNumElements());

      bool        Overflow;
      const APInt MaxNumElts = getVScaleRange(&F, BitWidth).getUnsignedMax().umul_ov(MinNumElts, Overflow);
      if (Overflow) {
        break;
      }
    ...
    case ISD::BUILD_VECTOR:
      ...
      for (unsigned i = 0, e = Op.getNumOperands(); i != e; ++i) {
        ...
        if (SrcOp.getValueSizeInBits() != BitWidth) {
          assert(SrcOp.getValueSizeInBits() > BitWidth && "Expected BUILD_VECTOR implicit truncation");
    ...
    case ISD::VECTOR_SHUFFLE: {
      ...
      if (!getShuffleDemandedElts(NumElts, SVN->getMask(), DemandedElts, DemandedLHS, DemandedRHS)) {
        break;
      }
    ...
    case ISD::CONCAT_VECTORS: {
      ...
      for (unsigned i = 0; i != NumSubVectors; ++i) {
        APInt DemandedSub = DemandedElts.extractBits(NumSubVectorElts, i * NumSubVectorElts);
        if (!!DemandedSub) {
    ...
    case ISD::BITCAST: {
      ...
      if ((SubBitWidth % BitWidth) == 0) {
        ...
        APInt    SubDemandedElts = APIntOps::ScaleBitMask(DemandedElts, NumElts / SubScale);
    ...
    case ISD::MUL: {
      ...
      if (SelfMultiply) {
        SelfMultiply &= isGuaranteedNotToBeUndefOrPoison(Op.getOperand(0), DemandedElts, false, Depth + 1);
      }
      ...
      if (Op->getFlags().hasNoSignedWrap() && Op.getOperand(0) == Op.getOperand(1) && !Known.isNegative()) {
    ...
    case ISD::ABDS: {
      ...
      unsigned SignBits1 = ComputeNumSignBits(Op.getOperand(1), DemandedElts, Depth + 1);
      if (SignBits1 == 1) {
        break;
      }
      unsigned SignBits0 = ComputeNumSignBits(Op.getOperand(0), DemandedElts, Depth + 1);
    ...
    case ISD::UMULO:
      ...
      if (TLI->getBooleanContents(Op.getValueType().isVector(), false) == TargetLowering::ZeroOrOneBooleanContent && BitWidth > 1) {
    ...
    case ISD::STRICT_FSETCCS: {
      ...
      // If we know the result of a setcc has the top bits zero, use this info.
      if (TLI->getBooleanContents(Op.getOperand(OpNo).getValueType()) == TargetLowering::ZeroOrOneBooleanContent && BitWidth > 1) {
        Known.Zero.setBitsFrom(1);
      }
      break;
    }
    case ISD::SHL: {
      ...
      if (std::optional<uint64_t> ShMinAmt = getValidMinimumShiftAmount(Op, DemandedElts, Depth + 1)) {
    ...
    case ISD::SRL:
      Known  = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
      Known2 = computeKnownBits(Op.getOperand(1), DemandedElts, Depth + 1);
      Known  = KnownBits::lshr(Known, Known2, /*ShAmtNonZero=*/false, Op->getFlags().hasExact());
    ...
    case ISD::SRA:
      Known  = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
      Known2 = computeKnownBits(Op.getOperand(1), DemandedElts, Depth + 1);
      Known  = KnownBits::ashr(Known, Known2, /*ShAmtNonZero=*/false, Op->getFlags().hasExact());
      break;
    case ISD::FSHL:
    case ISD::FSHR:
      if (ConstantSDNode* C = isConstOrConstSplat(Op.getOperand(2), DemandedElts)) {
        ...
        if (Amt == 0) {
          Known = computeKnownBits(Op.getOperand(Opcode == ISD::FSHL ? 0 : 1), DemandedElts, Depth + 1);
          break;
        }
    ...
    case ISD::MGATHER:
    case ISD::MLOAD: {
      ISD::LoadExtType ETy = (Opcode == ISD::MGATHER) ? cast<MaskedGatherSDNode>(Op)->getExtensionType()
    ...
    case ISD::LOAD: {
      ...
      if (ISD::isNON_EXTLoad(LD) && Cst) {
        ...
        if ((NumElts * BitWidth) == CstTy->getPrimitiveSizeInBits() && !Op.getValueType().isScalableVector()) {
          ...
          if (CstTy->isVectorTy() && BitWidth == CstTy->getScalarSizeInBits()) {
            ...
            if (auto* CInt = dyn_cast<ConstantInt>(Cst)) {
              ...
              Known = KnownBits::makeConstant(CFP->getValueAPF().bitcastToAPInt());
            }
          }
        }
    ...
    case ISD::SSUBO_CARRY:
      if (Op.getResNo() == 1) {
        // If we know the result of a setcc has the top bits zero, use this info.
        if (TLI->getBooleanContents(Op.getOperand(0).getValueType()) == TargetLowering::ZeroOrOneBooleanContent && BitWidth > 1) {
          Known.Zero.setBitsFrom(1);
        }
        break;
      }
      [[fallthrough]];
    case ISD::SUBC: {
      assert(Op.getResNo() == 0 && "We only compute knownbits for the difference here.");
    ...
    case ISD::EXTRACT_VECTOR_ELT: {
      ...
      if (ConstEltNo && ConstEltNo->getAPIntValue().ult(NumSrcElts)) {
        DemandedSrcElts = APInt::getOneBitSet(NumSrcElts, ConstEltNo->getZExtValue());
      }
    ...
    case ISD::ABS: {
      Known2 = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
      Known  = Known2.abs();
      Known.Zero.setHighBits(ComputeNumSignBits(Op.getOperand(0), DemandedElts, Depth + 1) - 1);
      break;
    }
    case ISD::USUBSAT: {
      Known  = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
    ...
    case ISD::SMAX: {
      ...
      if ((CstLow = isConstOrConstSplat(Op.getOperand(1), DemandedElts))) {
        if (Op.getOperand(0).getOpcode() == (IsMax ? ISD::SMIN : ISD::SMAX)) {
          CstHigh = isConstOrConstSplat(Op.getOperand(0).getOperand(1), DemandedElts);
        }
      }
      if (CstLow && CstHigh) {
        if (!IsMax) {
    ...
    case ISD::ATOMIC_LOAD: {
      ...
      if (Op.getResNo() == 0) {
        ...
        switch (AT->getExtensionType()) {
          case ISD::ZEXTLOAD: Known = KnownScalarMemory.zext(BitWidth); break;
          case ISD::SEXTLOAD: Known = KnownScalarMemory.sext(BitWidth); break;
          case ISD::EXTLOAD:
            switch (TLI->getExtendForAtomicOps()) {
              case ISD::ZERO_EXTEND: Known = KnownScalarMemory.zext(BitWidth); break;
              case ISD::SIGN_EXTEND: Known = KnownScalarMemory.sext(BitWidth); break;
              default: Known = KnownScalarMemory.anyext(BitWidth); break;
            }
            break;
          case ISD::NON_EXTLOAD: Known = KnownScalarMemory; break;
        }
        assert(Known.getBitWidth() == BitWidth);
      }
      break;
    ...
    case ISD::ATOMIC_CMP_SWAP_WITH_SUCCESS:
      if (Op.getResNo() == 1) {
        ...
        // We know that we have an integer-based boolean since these operations
        // are only available for integer.
        if (TLI->getBooleanContents(Op.getValueType().isVector(), false) == TargetLowering::ZeroOrOneBooleanContent
            && BitWidth > 1) {
          Known.Zero.setBitsFrom(1);
        }
        break;
    ...
    case ISD::TargetFrameIndex:
      TLI->computeKnownBitsForFrameIndex(cast<FrameIndexSDNode>(Op)->getIndex(), Known, getMachineFunction());
      break;
...
static SelectionDAG::OverflowKind mapOverflowResult(ConstantRange::OverflowResult OR) {
  switch (OR) {
    case ConstantRange::OverflowResult::MayOverflow: return SelectionDAG::OFK_Sometime;
    case ConstantRange::OverflowResult::AlwaysOverflowsLow:
    case ConstantRange::OverflowResult::AlwaysOverflowsHigh: return SelectionDAG::OFK_Always;
    case ConstantRange::OverflowResult::NeverOverflows: return SelectionDAG::OFK_Never;
  }
  llvm_unreachable("Unknown OverflowResult");
}

SelectionDAG::OverflowKind SelectionDAG::computeOverflowForSignedAdd(SDValue N0, SDValue N1) const {
  // X + 0 never overflow
...
SelectionDAG::OverflowKind SelectionDAG::computeOverflowForUnsignedAdd(SDValue N0, SDValue N1) const {
  // X + 0 never overflow
  if (isNullConstant(N1)) {
    return OFK_Never;
  }
  ...
  if (N0.getOpcode() == ISD::UMUL_LOHI && N0.getResNo() == 1 && N1Known.getMaxValue().ult(2)) {
    return OFK_Never;
  }

  KnownBits N0Known = computeKnownBits(N0);
  if (N1.getOpcode() == ISD::UMUL_LOHI && N1.getResNo() == 1 && N0Known.getMaxValue().ult(2)) {
    return OFK_Never;
  }
...
SelectionDAG::OverflowKind SelectionDAG::computeOverflowForSignedSub(SDValue N0, SDValue N1) const {
  // X - 0 never overflow
  if (isNullConstant(N1)) {
    return OFK_Never;
  }
...
SelectionDAG::OverflowKind SelectionDAG::computeOverflowForUnsignedSub(SDValue N0, SDValue N1) const {
  // X - 0 never overflow
  if (isNullConstant(N1)) {
    return OFK_Never;
  }
...
SelectionDAG::OverflowKind SelectionDAG::computeOverflowForUnsignedMul(SDValue N0, SDValue N1) const {
...
SelectionDAG::OverflowKind SelectionDAG::computeOverflowForSignedMul(SDValue N0, SDValue N1) const {
  // X * 0 and X * 1 never overflow.
  if (isNullConstant(N1) || isOneConstant(N1)) {
    return OFK_Never;
  }
...
bool SelectionDAG::isKnownToBeAPowerOfTwo(SDValue Val, unsigned Depth) const {
  ...
  if (ISD::matchUnaryPredicate(Val,
                               [BitWidth](ConstantSDNode* C) { return C->getAPIntValue().zextOrTrunc(BitWidth).isPowerOf2(); })) {
    return true;
  }
  ...
  if (Val.getOpcode() == ISD::SHL) {
    ...
    return isKnownToBeAPowerOfTwo(Val.getOperand(0), Depth + 1) && isKnownNeverZero(Val, Depth);
  }
  ...
  if (Val.getOpcode() == ISD::VSCALE && getTargetLoweringInfo().isVScaleKnownToBeAPowerOfTwo()
      && isKnownToBeAPowerOfTwo(Val.getOperand(0), Depth + 1)) {
    return true;
  }

  if (Val.getOpcode() == ISD::SMIN || Val.getOpcode() == ISD::SMAX || Val.getOpcode() == ISD::UMIN
      || Val.getOpcode() == ISD::UMAX) {
    return isKnownToBeAPowerOfTwo(Val.getOperand(1), Depth + 1) && isKnownToBeAPowerOfTwo(Val.getOperand(0), Depth + 1);
  }

  if (Val.getOpcode() == ISD::SELECT || Val.getOpcode() == ISD::VSELECT) {
    return isKnownToBeAPowerOfTwo(Val.getOperand(2), Depth + 1) && isKnownToBeAPowerOfTwo(Val.getOperand(1), Depth + 1);
  }
...
unsigned SelectionDAG::ComputeNumSignBits(SDValue Op, const APInt& DemandedElts, unsigned Depth) const {
  EVT VT = Op.getValueType();
  ...
  switch (Opcode) {
    default: break;
    case ISD::AssertSext: Tmp = cast<VTSDNode>(Op.getOperand(1))->getVT().getSizeInBits(); return VTBits - Tmp + 1;
    case ISD::AssertZext: Tmp = cast<VTSDNode>(Op.getOperand(1))->getVT().getSizeInBits(); return VTBits - Tmp;
    case ISD::MERGE_VALUES: return ComputeNumSignBits(Op.getOperand(Op.getResNo()), DemandedElts, Depth + 1);
    case ISD::SPLAT_VECTOR: {
      ...
      for (unsigned i = 0, e = Op.getNumOperands(); (i < e) && (Tmp > 1); ++i) {
        ...
        if (ConstantSDNode* C = dyn_cast<ConstantSDNode>(SrcOp)) {
          ...
          if (SrcOp.getValueSizeInBits() != VTBits) {
            assert(SrcOp.getValueSizeInBits() > VTBits && "Expected BUILD_VECTOR implicit truncation");
    ...
    case ISD::BITCAST: {
      ...
      if ((SrcBits % VTBits) == 0) {
        ...
        APInt    SrcDemandedElts = APIntOps::ScaleBitMask(DemandedElts, NumElts / Scale);
    ...
    case ISD::SRA:
      ...
      if (std::optional<uint64_t> ShAmt = getValidMinimumShiftAmount(Op, DemandedElts, Depth + 1)) {
        Tmp = std::min<uint64_t>(Tmp + *ShAmt, VTBits);
      }
      return Tmp;
    case ISD::SHL:
      if (std::optional<ConstantRange> ShAmtRange = getValidShiftAmountRange(Op, DemandedElts, Depth + 1)) {
        ...
        if (ISD::isExtOpcode(Op.getOperand(0).getOpcode())) {
          ...
          uint64_t SizeDifference = ExtVT.getScalarSizeInBits() - ExtendeeVT.getScalarSizeInBits();
          if (SizeDifference <= MinShAmt) {
            Tmp = SizeDifference + ComputeNumSignBits(Extendee, DemandedElts, Depth + 1);
            if (MaxShAmt < Tmp) {
              return Tmp - MaxShAmt;
            }
          }
    ...
    case ISD::UMULO:
      ...
      // We know that we have an integer-based boolean since these operations
      // are only available for integer.
      if (TLI->getBooleanContents(VT.isVector(), false) == TargetLowering::ZeroOrNegativeOneBooleanContent) {
        return VTBits;
      }
      break;
    case ISD::SETCC:
    ...
    case ISD::STRICT_FSETCCS: {
      unsigned OpNo = Op->isStrictFPOpcode() ? 1 : 0;
      // If setcc returns 0/-1, all bits are sign bits.
      if (TLI->getBooleanContents(Op.getOperand(OpNo).getValueType()) == TargetLowering::ZeroOrNegativeOneBooleanContent) {
        return VTBits;
      }
      break;
    }
    ...
    case ISD::ADDC:
      ...
      if (ConstantSDNode* CRHS = isConstOrConstSplat(Op.getOperand(1), DemandedElts)) {
        if (CRHS->isAllOnes()) {
          KnownBits Known = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
    ...
    case ISD::SUB:
      ...
      // Handle NEG.
      if (ConstantSDNode* CLHS = isConstOrConstSplat(Op.getOperand(0), DemandedElts)) {
        if (CLHS->isZero()) {
          KnownBits Known = computeKnownBits(Op.getOperand(1), DemandedElts, Depth + 1);
          // If the input is known to be 0 or 1, the output is 0/-1, which is all
          // sign bits set.
          if ((Known.Zero | 1).isAllOnes()) {
            return VTBits;
    ...
    case ISD::MUL: {
      ...
      unsigned OutValidBits = (VTBits - SignBitsOp0 + 1) + (VTBits - SignBitsOp1 + 1);
      ...
      if (const MDNode* Ranges = LD->getRanges()) {
        ...
        if (VTBits > CR.getBitWidth()) {
          switch (LD->getExtensionType()) {
            case ISD::SEXTLOAD: CR = CR.signExtend(VTBits); break;
            case ISD::ZEXTLOAD: CR = CR.zeroExtend(VTBits); break;
            default: break;
          }
        }
        ...
        return std::min(CR.getSignedMin().getNumSignBits(), CR.getSignedMax().getNumSignBits());
      }
    ...
    case ISD::ATOMIC_LOAD: {
      ...
      if (Op.getResNo() == 0) {
        ...
        if (Op->getOpcode() == ISD::ATOMIC_LOAD) {
          switch (AT->getExtensionType()) {
            default: break;
            case ISD::SEXTLOAD: return VTBits - Tmp + 1;
            case ISD::ZEXTLOAD: return VTBits - Tmp;
          }
        }
      ...
      switch (ExtType) {
        ...
        case ISD::NON_EXTLOAD:
          if (const Constant* Cst = TLI->getTargetConstantFromLoad(LD)) {
            ...
            // scalar cases.
            Type* CstTy = Cst->getType();
            if (CstTy->isVectorTy() && !VT.isScalableVector() && (NumElts * VTBits) == CstTy->getPrimitiveSizeInBits()
  ...
  if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN || Opcode == ISD::INTRINSIC_W_CHAIN
    ...
    if (!VT.isScalableVector()) {
      unsigned NumBits = TLI->ComputeNumSignBitsForTargetNode(Op, DemandedElts, *this, Depth);
      if (NumBits > 1) {
...
unsigned SelectionDAG::ComputeMaxSignificantBits(SDValue Op, unsigned Depth) const {
  unsigned SignBits = ComputeNumSignBits(Op, Depth);
  return Op.getScalarValueSizeInBits() - SignBits + 1;
}

unsigned SelectionDAG::ComputeMaxSignificantBits(SDValue Op, const APInt& DemandedElts, unsigned Depth) const {
  unsigned SignBits = ComputeNumSignBits(Op, DemandedElts, Depth);
  return Op.getScalarValueSizeInBits() - SignBits + 1;
}

bool SelectionDAG::isGuaranteedNotToBeUndefOrPoison(SDValue Op, bool PoisonOnly, unsigned Depth) const {
...
bool SelectionDAG::isGuaranteedNotToBeUndefOrPoison(SDValue Op, const APInt& DemandedElts, bool PoisonOnly, unsigned Depth) const {
  unsigned Opcode = Op.getOpcode();
  ...
  switch (Opcode) {
    ...
    case ISD::FrameIndex:
    case ISD::TargetFrameIndex:
    case ISD::CopyFromReg: return true;

    case ISD::POISON: return false;

    case ISD::UNDEF: return PoisonOnly;
    ...
    case ISD::BUILD_VECTOR:
      ...
      for (unsigned i = 0, e = Op.getNumOperands(); i < e; ++i) {
        ...
        if (!isGuaranteedNotToBeUndefOrPoison(Op.getOperand(i), PoisonOnly, Depth + 1)) {
          return false;
        }
      }
      return true;

    case ISD::SPLAT_VECTOR: return isGuaranteedNotToBeUndefOrPoison(Op.getOperand(0), PoisonOnly, Depth + 1);

    case ISD::VECTOR_SHUFFLE: {
      APInt DemandedLHS, DemandedRHS;
      auto* SVN = cast<ShuffleVectorSDNode>(Op);
      if (!getShuffleDemandedElts(DemandedElts.getBitWidth(),
                                  SVN->getMask(),
                                  DemandedElts,
                                  DemandedLHS,
                                  DemandedRHS,
      ...
      if (!DemandedLHS.isZero() && !isGuaranteedNotToBeUndefOrPoison(Op.getOperand(0), DemandedLHS, PoisonOnly, Depth + 1)) {
        return false;
      }
      if (!DemandedRHS.isZero() && !isGuaranteedNotToBeUndefOrPoison(Op.getOperand(1), DemandedRHS, PoisonOnly, Depth + 1)) {
        return false;
      }
      return true;
    }
    ...
    default:
      ...
      if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN || Opcode == ISD::INTRINSIC_W_CHAIN
          || Opcode == ISD::INTRINSIC_VOID) {
        return TLI->isGuaranteedNotToBeUndefOrPoisonForTargetNode(Op, DemandedElts, *this, PoisonOnly, Depth);
      }
      break;
  }
  ...
  return !canCreateUndefOrPoison(Op, PoisonOnly, /*ConsiderFlags*/ true, Depth)
         && all_of(Op->ops(), [&](SDValue V) { return isGuaranteedNotToBeUndefOrPoison(V, PoisonOnly, Depth + 1); });
}

bool SelectionDAG::canCreateUndefOrPoison(SDValue Op, bool PoisonOnly, bool ConsiderFlags, unsigned Depth) const {
  EVT   VT           = Op.getValueType();
  APInt DemandedElts = VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorNumElements()) : APInt(1, 1);
  return canCreateUndefOrPoison(Op, DemandedElts, PoisonOnly, ConsiderFlags, Depth);
}

bool SelectionDAG::canCreateUndefOrPoison(SDValue Op, const APInt& DemandedElts, bool PoisonOnly, bool ConsiderFlags,
  ...
  switch (Opcode) {
    ...
    case ISD::BUILD_VECTOR:
    case ISD::BUILD_PAIR:
    case ISD::SPLAT_VECTOR: return false;
    ...
    case ISD::SRA:
      ...
      // create poison.
      return !isGuaranteedNotToBeUndefOrPoison(Op.getOperand(1), DemandedElts, PoisonOnly, Depth + 1)
    ...
    case ISD::EXTRACT_VECTOR_ELT: {
      ...
      if (isGuaranteedNotToBeUndefOrPoison(Idx, DemandedElts, PoisonOnly, Depth + 1)) {
    ...
    default:
      ...
      if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN || Opcode == ISD::INTRINSIC_W_CHAIN
          || Opcode == ISD::INTRINSIC_VOID) {
        return TLI->canCreateUndefOrPoisonForTargetNode(Op, DemandedElts, *this, PoisonOnly, ConsiderFlags, Depth);
      }
      break;
  }
...
bool SelectionDAG::isADDLike(SDValue Op, bool NoWrap) const {
  unsigned Opcode = Op.getOpcode();
  if (Opcode == ISD::OR) {
    return Op->getFlags().hasDisjoint() || haveNoCommonBitsSet(Op.getOperand(0), Op.getOperand(1));
  }
  if (Opcode == ISD::XOR) {
...
bool SelectionDAG::isBaseWithConstantOffset(SDValue Op) const {
  return Op.getNumOperands() == 2 && isa<ConstantSDNode>(Op.getOperand(1)) && (Op.getOpcode() == ISD::ADD || isADDLike(Op));
}
...
bool SelectionDAG::isKnownNeverNaN(SDValue Op, const APInt& DemandedElts, bool SNaN, unsigned Depth) const {
  ...
  if (const ConstantFPSDNode* C = dyn_cast<ConstantFPSDNode>(Op)) {
    return !C->getValueAPF().isNaN() || (SNaN && !C->getValueAPF().isSignaling());
  }
  ...
  switch (Opcode) {
    ...
    case ISD::SINT_TO_FP:
    case ISD::UINT_TO_FP: return true;
    ...
    case ISD::EXTRACT_VECTOR_ELT: {
      ...
      EVT     SrcVT = Src.getValueType();
      if (SrcVT.isFixedLengthVector() && Idx && Idx->getAPIntValue().ult(SrcVT.getVectorNumElements())) {
        APInt DemandedSrcElts = APInt::getOneBitSet(SrcVT.getVectorNumElements(), Idx->getZExtValue());
    ...
    case ISD::INSERT_SUBVECTOR: {
      ...
      if (BaseVectorVT.isFixedLengthVector()) {
        ...
        // inserted.
        APInt DemandedMask    = APInt::getBitsSet(NumBaseElts, Idx, Idx + NumSubElts);
        APInt DemandedSrcElts = DemandedElts & ~DemandedMask;
        APInt DemandedSubElts = DemandedElts.extractBits(NumSubElts, Idx);

        bool NeverNaN = true;
        if (!DemandedSrcElts.isZero()) {
          NeverNaN &= isKnownNeverNaN(BaseVector, DemandedSrcElts, SNaN, Depth + 1);
        }
        if (NeverNaN && !DemandedSubElts.isZero()) {
          NeverNaN &= isKnownNeverNaN(SubVector, DemandedSubElts, SNaN, Depth + 1);
        }
        return NeverNaN;
      }
      return isKnownNeverNaN(BaseVector, SNaN, Depth + 1) && isKnownNeverNaN(SubVector, SNaN, Depth + 1);
    }
    case ISD::BUILD_VECTOR: {
      unsigned NumElts = Op.getNumOperands();
      for (unsigned I = 0; I != NumElts; ++I) {
        if (DemandedElts[I] && !isKnownNeverNaN(Op.getOperand(I), SNaN, Depth + 1)) {
          return false;
        }
      }
      return true;
    ...
    case ISD::AssertNoFPClass: {
      FPClassTest NoFPClass = static_cast<FPClassTest>(Op.getConstantOperandVal(1));
    ...
    default:
      if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN || Opcode == ISD::INTRINSIC_W_CHAIN
          || Opcode == ISD::INTRINSIC_VOID) {
        return TLI->isKnownNeverNaNForTargetNode(Op, DemandedElts, *this, SNaN, Depth);
      }
...
bool SelectionDAG::isKnownNeverZeroFloat(SDValue Op) const {
  ...
  return ISD::matchUnaryFpPredicate(Op, [](ConstantFPSDNode* C) { return !C->isZero(); });
}
...
bool SelectionDAG::isKnownNeverZero(SDValue Op, unsigned Depth) const {
  ...
  assert(!Op.getValueType().isFloatingPoint() && "Floating point types unsupported - use isKnownNeverZeroFloat");

  // If the value is a constant, we can obviously see if it is a zero or not.
  if (ISD::matchUnaryPredicate(Op, [](ConstantSDNode* C) { return !C->isZero(); })) {
    return true;
  }
  ...
  switch (Op.getOpcode()) {
    ...
    case ISD::OR: return isKnownNeverZero(Op.getOperand(1), Depth + 1) || isKnownNeverZero(Op.getOperand(0), Depth + 1);

    case ISD::VSELECT:
    case ISD::SELECT: return isKnownNeverZero(Op.getOperand(1), Depth + 1) && isKnownNeverZero(Op.getOperand(2), Depth + 1);
    ...
    case ISD::SHL: {
      ...
      if (MaxCnt.ult(ValKnown.getBitWidth()) && !ValKnown.One.shl(MaxCnt).isZero()) {
        return true;
      }
      break;
    }
    case ISD::UADDSAT:
    case ISD::UMAX:
      return isKnownNeverZero(Op.getOperand(1), Depth + 1) || isKnownNeverZero(Op.getOperand(0), Depth + 1);
    ...
    case ISD::UMIN: return isKnownNeverZero(Op.getOperand(1), Depth + 1) && isKnownNeverZero(Op.getOperand(0), Depth + 1);
    ...
    case ISD::BSWAP:
    case ISD::CTPOP:
    case ISD::ABS: return isKnownNeverZero(Op.getOperand(0), Depth + 1);
    ...
    case ISD::SRL: {
      ...
      // If max shift cnt of known ones is non-zero, result is non-zero.
      APInt MaxCnt = computeKnownBits(Op.getOperand(1), Depth + 1).getMaxValue();
      if (MaxCnt.ult(ValKnown.getBitWidth()) && !ValKnown.One.lshr(MaxCnt).isZero()) {
        return true;
      }
      break;
    }
    ...
    case ISD::ADD:
      if (Op->getFlags().hasNoUnsignedWrap()) {
        if (isKnownNeverZero(Op.getOperand(1), Depth + 1) || isKnownNeverZero(Op.getOperand(0), Depth + 1)) {
          return true;
        }
      }
    ...
    case ISD::SUB: {
      ...
      std::optional<bool> ne
          = KnownBits::ne(computeKnownBits(Op.getOperand(0), Depth + 1), computeKnownBits(Op.getOperand(1), Depth + 1));
      return ne && *ne;
    }
    ...
    case ISD::ZERO_EXTEND:
    case ISD::SIGN_EXTEND: return isKnownNeverZero(Op.getOperand(0), Depth + 1);
    case ISD::VSCALE: {
      const Function& F          = getMachineFunction().getFunction();
      const APInt&    Multiplier = Op.getConstantOperandAPInt(0);
      ConstantRange   CR         = getVScaleRange(&F, Op.getScalarValueSizeInBits()).multiply(Multiplier);
...
SDValue llvm::getBitwiseNotOperand(SDValue V, SDValue Mask, bool AllowUndefs) {
  ...
  if (ExtArg.getScalarValueSizeInBits() >= MaskC->getAPIntValue().getActiveBits() && isBitwiseNot(ExtArg, AllowUndefs)
...
static bool haveNoCommonBitsSetCommutative(SDValue A, SDValue B) {
  ...
  auto MatchNoCommonBitsPattern = [&](SDValue Not, SDValue Mask, SDValue Other) {
    if (SDValue NotOperand = getBitwiseNotOperand(Not, Mask, /* AllowUndefs */ true)) {
      if (NotOperand->getOpcode() == ISD::ZERO_EXTEND || NotOperand->getOpcode() == ISD::TRUNCATE) {
      ...
      if (Other->getOpcode() == ISD::AND) {
        return NotOperand == Other->getOperand(0) || NotOperand == Other->getOperand(1);
      }
    }
    return false;
  };
...
bool SelectionDAG::haveNoCommonBitsSet(SDValue A, SDValue B) const {
  assert(A.getValueType() == B.getValueType() && "Values must have the same type");
  if (haveNoCommonBitsSetCommutative(A, B) || haveNoCommonBitsSetCommutative(B, A)) {
    return true;
  }
  return KnownBits::haveNoCommonBitsSet(computeKnownBits(A), computeKnownBits(B));
}
...
static SDValue FoldBUILD_VECTOR(const SDLoc& DL, EVT VT, ArrayRef<SDValue> Ops, SelectionDAG& DAG) {
  ...
  assert(!VT.isScalableVector() && "BUILD_VECTOR cannot be used with scalable types");
  assert(VT.getVectorNumElements() == (unsigned)NumOps && "Incorrect element count in BUILD_VECTOR!");
  ...
  for (int i = 0; i != NumOps; ++i) {
    if (Ops[i].getOpcode() != ISD::EXTRACT_VECTOR_ELT || Ops[i].getOperand(0).getValueType() != VT
        || (IdentitySrc && Ops[i].getOperand(0) != IdentitySrc) || !isa<ConstantSDNode>(Ops[i].getOperand(1))
...
static SDValue foldCONCAT_VECTORS(const SDLoc& DL, EVT VT, ArrayRef<SDValue> Ops, SelectionDAG& DAG) {
  ...
  assert(llvm::all_of(Ops, [Ops](SDValue Op) { return Ops[0].getValueType() == Op.getValueType(); })
         && "Concatenation of vectors with inconsistent value types!");
  assert((Ops[0].getValueType().getVectorElementCount() * Ops.size()) == VT.getVectorElementCount()
  ...
  for (unsigned i = 0, e = Ops.size(); i != e; ++i) {
    ...
    if (Op.getOpcode() != ISD::EXTRACT_SUBVECTOR || Op.getOperand(0).getValueType() != VT
        || (IdentitySrc && Op.getOperand(0) != IdentitySrc) || Op.getConstantOperandVal(1) != IdentityIndex) {
      IsIdentity = false;
      break;
    }
    assert((!IdentitySrc || IdentitySrc == Op.getOperand(0)) && "Unexpected identity source vector for concat of extracts");
  ...
  if (SVT.bitsGT(VT.getScalarType())) {
    for (SDValue& Op : Elts) {
      if (Op.isUndef()) {
        ...
        Op = DAG.getTargetLoweringInfo().isZExtFree(Op.getValueType(), SVT) ? DAG.getZExtOrTrunc(Op, DL, SVT)
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1) {
  SDNodeFlags Flags;
  if (Inserter) {
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, const SDNodeFlags Flags) {
  ...
  switch (Opcode) {
    case ISD::STEP_VECTOR:
      assert(VT.isScalableVector() && "STEP_VECTOR can only be used with scalable types");
      assert(OpOpcode == ISD::TargetConstant && VT.getVectorElementType() == N1.getValueType() && "Unexpected step operand");
      break;
    case ISD::FREEZE:
      assert(VT == N1.getValueType() && "Unexpected VT!");
      if (isGuaranteedNotToBeUndefOrPoison(N1,
                                           /*PoisonOnly*/ false,
                                           /*Depth*/ 1)) {
        return N1;
      }
    ...
    case ISD::CONCAT_VECTORS: return N1;  // Factor, merge or concat of one node?  No need.
    case ISD::BUILD_VECTOR: {
    ...
    case ISD::FP_EXTEND:
      assert(VT.isFloatingPoint() && N1.getValueType().isFloatingPoint() && "Invalid FP cast!");
      if (N1.getValueType() == VT) {
        return N1;  // noop conversion.
      }
      assert((!VT.isVector() || VT.getVectorElementCount() == N1.getValueType().getVectorElementCount())
    ...
    case ISD::UINT_TO_FP:
      ...
      break;
    case ISD::SIGN_EXTEND:
      assert(VT.isInteger() && N1.getValueType().isInteger() && "Invalid SIGN_EXTEND!");
      ...
      break;
    case ISD::ZERO_EXTEND:
      assert(VT.isInteger() && N1.getValueType().isInteger() && "Invalid ZERO_EXTEND!");
      assert(VT.isVector() == N1.getValueType().isVector() &&
      ...
      if (OpOpcode == ISD::TRUNCATE) {
        ...
        if (OpOp.getValueType() == VT) {
          if (OpOp.getOpcode() != ISD::AND) {
            APInt HiBits = APInt::getBitsSetFrom(VT.getScalarSizeInBits(), N1.getScalarValueSizeInBits());
      ...
      break;
    case ISD::ANY_EXTEND:
      assert(VT.isInteger() && N1.getValueType().isInteger() && "Invalid ANY_EXTEND!");
      assert(VT.isVector() == N1.getValueType().isVector() &&
      ...
      break;
    case ISD::TRUNCATE:
      assert(VT.isInteger() && N1.getValueType().isInteger() && "Invalid TRUNCATE!");
      assert(VT.isVector() == N1.getValueType().isVector() &&
      ...
      if (OpOpcode == ISD::ZERO_EXTEND || OpOpcode == ISD::SIGN_EXTEND || OpOpcode == ISD::ANY_EXTEND) {
        ...
        if (N1.getOperand(0).getValueType().getScalarType().bitsLT(VT.getScalarType())) {
      ...
      if (OpOpcode == ISD::VSCALE && !NewNodesMustHaveLegalTypes) {
        return getVScale(DL, VT, N1.getConstantOperandAPInt(0).trunc(VT.getSizeInBits()));
      }
      break;
    case ISD::ANY_EXTEND_VECTOR_INREG:
    case ISD::ZERO_EXTEND_VECTOR_INREG:
    case ISD::SIGN_EXTEND_VECTOR_INREG:
      ...
      assert(N1.getValueType().bitsLE(VT) && "The input must be the same size or smaller than the result.");
      assert(VT.getVectorMinNumElements() < N1.getValueType().getVectorMinNumElements()
    ...
    case ISD::BSWAP:
      ...
      assert((VT.getScalarSizeInBits() % 16 == 0) && "BSWAP types must be a multiple of 16 bits!");
      if (N1.isUndef()) {
        return getUNDEF(VT);
      }
      // bswap(bswap(X)) -> X.
    ...
    case ISD::BITREVERSE:
      ...
      break;
    case ISD::BITCAST:
      assert(VT.getSizeInBits() == N1.getValueSizeInBits() && "Cannot BITCAST between types of different sizes!");
    ...
    case ISD::SCALAR_TO_VECTOR:
      assert(VT.isVector() && !N1.getValueType().isVector()
             && (VT.getVectorElementType() == N1.getValueType()
                 || (VT.getVectorElementType().isInteger() && N1.getValueType().isInteger()
      ...
      if (OpOpcode == ISD::EXTRACT_VECTOR_ELT && isa<ConstantSDNode>(N1.getOperand(1)) && N1.getConstantOperandVal(1) == 0
    ...
    case ISD::FABS:
      ...
      break;
    case ISD::VSCALE: assert(VT == N1.getValueType() && "Unexpected VT!"); break;
    case ISD::CTPOP:
    ...
    case ISD::SPLAT_VECTOR:
      ...
      // that for now.
      assert((VT.getVectorElementType() == N1.getValueType() || (VT.isFloatingPoint() && N1.getValueType() == MVT::i32)
              || (VT.getVectorElementType().isInteger() && N1.getValueType().isInteger()
                  && VT.getVectorElementType().bitsLE(N1.getValueType())))
             && "Wrong operand type!");
      break;
...
static std::optional<APInt> FoldValue(unsigned Opcode, const APInt& C1, const APInt& C2) {
  switch (Opcode) {
    ...
    case ISD::SREM:
      ...
      return C1.srem(C2);
    case ISD::AVGFLOORS: return APIntOps::avgFloorS(C1, C2);
    case ISD::AVGFLOORU: return APIntOps::avgFloorU(C1, C2);
    case ISD::AVGCEILS: return APIntOps::avgCeilS(C1, C2);
    case ISD::AVGCEILU: return APIntOps::avgCeilU(C1, C2);
    case ISD::ABDS: return APIntOps::abds(C1, C2);
    case ISD::ABDU: return APIntOps::abdu(C1, C2);
...
SDValue SelectionDAG::FoldSymbolOffset(unsigned Opcode, EVT VT, const GlobalAddressSDNode* GA, const SDNode* N2) {
  ...
  return getGlobalAddress(GA->getGlobal(), SDLoc(C2), VT, GA->getOffset() + uint64_t(Offset));
}
  ...
  switch (Opcode) {
    ...
    case ISD::UREM: {
      ...
      return ISD::isBuildVectorOfConstantSDNodes(Divisor.getNode())
             && llvm::any_of(Divisor->op_values(), [](SDValue V) { return V.isUndef() || isNullConstant(V); });
...
SDValue SelectionDAG::FoldConstantArithmetic(unsigned Opcode, const SDLoc& DL, EVT VT, ArrayRef<SDValue> Ops, SDNodeFlags Flags) {
  ...
  if (NumOps == 1) {
    ...
    if (auto* C = dyn_cast<ConstantSDNode>(N1)) {
      const APInt& Val = C->getAPIntValue();
      switch (Opcode) {
        case ISD::SIGN_EXTEND: return getConstant(Val.sextOrTrunc(VT.getSizeInBits()), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::TRUNCATE:
          if (C->isOpaque()) {
            break;
          }
        ...
        case ISD::ANY_EXTEND:
          ...
          if (TLI->isSExtCheaperThanZExt(N1.getValueType(), VT)) {
            return getConstant(Val.sextOrTrunc(VT.getSizeInBits()), DL, VT, C->isTargetOpcode(), C->isOpaque());
          }
          return getConstant(Val.zextOrTrunc(VT.getSizeInBits()), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::ABS: return getConstant(Val.abs(), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::BITREVERSE: return getConstant(Val.reverseBits(), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::BSWAP: return getConstant(Val.byteSwap(), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::CTPOP: return getConstant(Val.popcount(), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::CTLZ:
        case ISD::CTLZ_ZERO_UNDEF: return getConstant(Val.countl_zero(), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::CTTZ:
        case ISD::CTTZ_ZERO_UNDEF: return getConstant(Val.countr_zero(), DL, VT, C->isTargetOpcode(), C->isOpaque());
        case ISD::UINT_TO_FP:
        case ISD::SINT_TO_FP: {
          APFloat FPV(VT.getFltSemantics(), APInt::getZero(VT.getSizeInBits()));
          (void)FPV.convertFromAPInt(Val, Opcode == ISD::SINT_TO_FP, APFloat::rmNearestTiesToEven);
          return getConstantFP(FPV, DL, VT);
        }
        case ISD::FP16_TO_FP:
        case ISD::BF16_TO_FP: {
          bool    Ignored;
          APFloat FPV(Opcode == ISD::FP16_TO_FP ? APFloat::IEEEhalf() : APFloat::BFloat(),
                      (Val.getBitWidth() == 16) ? Val : Val.trunc(16));

          // This can return overflow, underflow, or inexact; we don't care.
          // FIXME need to be more flexible about rounding mode.
          (void)FPV.convert(VT.getFltSemantics(), APFloat::rmNearestTiesToEven, &Ignored);
          return getConstantFP(FPV, DL, VT);
        }
        case ISD::STEP_VECTOR:
    ...
    if (auto* C = dyn_cast<ConstantFPSDNode>(N1)) {
      ...
      switch (Opcode) {
        case ISD::FNEG: V.changeSign(); return getConstantFP(V, DL, VT);
        case ISD::FABS: V.clearSign(); return getConstantFP(V, DL, VT);
        case ISD::FCEIL: {
        ...
        case ISD::FP_EXTEND: {
          ...
          // This can return overflow, underflow, or inexact; we don't care.
          // FIXME need to be more flexible about rounding mode.
          (void)V.convert(VT.getFltSemantics(), APFloat::rmNearestTiesToEven, &ignored);
          return getConstantFP(V, DL, VT);
        }
        case ISD::FP_TO_SINT:
        case ISD::FP_TO_UINT: {
          ...
          // FIXME need to be more flexible about rounding mode.
          APFloat::opStatus s = V.convertToInteger(IntVal, APFloat::rmTowardZero, &ignored);
        ...
        case ISD::FP_TO_BF16: {
          ...
          // This can return overflow, underflow, or inexact; we don't care.
          // FIXME need to be more flexible about rounding mode.
          (void)V.convert(Opcode == ISD::FP_TO_FP16 ? APFloat::IEEEhalf() : APFloat::BFloat(),
  ...
  if (NumOps == 2) {
    ...
    if (auto* C1 = dyn_cast<ConstantSDNode>(Ops[0])) {
      if (auto* C2 = dyn_cast<ConstantSDNode>(Ops[1])) {
        ...
        std::optional<APInt> FoldAttempt = FoldValue(Opcode, C1->getAPIntValue(), C2->getAPIntValue());
        if (!FoldAttempt) {
          return SDValue();
        }

        SDValue Folded = getConstant(*FoldAttempt, DL, VT);
        assert((!Folded || !VT.isVector()) && "Can't fold vectors ops with scalar operands");
        return Folded;
      }
    }
    ...
    if (Opcode == ISD::SIGN_EXTEND_INREG) {
      ...
      if (Ops[0].getOpcode() == ISD::SPLAT_VECTOR && isa<ConstantSDNode>(Ops[0].getOperand(0))) {
        return getNode(ISD::SPLAT_VECTOR,
                       DL,
                       VT,
                       SignExtendInReg(Ops[0].getConstantOperandAPInt(0), Ops[0].getOperand(0).getValueType()));
      }
    }
  }
  ...
  if (NumOps == 2 && VT.isFixedLengthVector() && VT.isInteger() && Ops[0].getValueType() == VT && Ops[1].getValueType() == VT
      && (Ops[0].getOpcode() == ISD::BITCAST || Ops[1].getOpcode() == ISD::BITCAST)) {
    SDValue N1  = peekThroughBitcasts(Ops[0]);
    SDValue N2  = peekThroughBitcasts(Ops[1]);
    auto*   BV1 = dyn_cast<BuildVectorSDNode>(N1);
    auto*   BV2 = dyn_cast<BuildVectorSDNode>(N2);
    if (BV1 && BV2 && N1.getValueType().isInteger() && N2.getValueType().isInteger()) {
      bool               IsLE    = getDataLayout().isLittleEndian();
      ...
      if (BV1->getConstantRawBits(IsLE, EltBits, RawBits1, UndefElts1)
        ...
        for (unsigned I = 0, E = NumElts.getFixedValue(); I != E; ++I) {
          std::optional<APInt> Fold = FoldValueWithUndef(Opcode, RawBits1[I], UndefElts1[I], RawBits2[I], UndefElts2[I]);
          if (!Fold) {
            break;
          }
        ...
        if (RawBits.size() == NumElts.getFixedValue()) {
          ...
          BitVector          DstUndefs;
          BuildVectorSDNode::recastRawBits(IsLE,
                                           BVVT.getScalarSizeInBits(),
                                           DstBits,
                                           RawBits,
                                           DstUndefs,
  ...
  if ((Opcode == ISD::MUL || Opcode == ISD::SHL) && Ops[0].getOpcode() == ISD::STEP_VECTOR) {
    APInt RHSVal;
    if (ISD::isConstantSplatVector(Ops[1].getNode(), RHSVal)) {
      APInt NewStep = Opcode == ISD::MUL ? Ops[0].getConstantOperandAPInt(0) * RHSVal : Ops[0].getConstantOperandAPInt(0) << RHSVal;
      return getStepVector(DL, VT, NewStep);
    }
  }

  auto IsScalarOrSameVectorSize = [NumElts](const SDValue& Op) {
    return !Op.getValueType().isVector() || Op.getValueType().getVectorElementCount() == NumElts;
  };

  auto IsBuildVectorSplatVectorOrUndef = [](const SDValue& Op) {
    return Op.isUndef() || Op.getOpcode() == ISD::CONDCODE || Op.getOpcode() == ISD::BUILD_VECTOR
  ...
  // or UNDEF scalars.
  if (!llvm::all_of(Ops, IsBuildVectorSplatVectorOrUndef) || !llvm::all_of(Ops, IsScalarOrSameVectorSize)) {
    return SDValue();
  }
  ...
  ISD::NodeType ExtendCode = (Opcode == ISD::SETCC && SVT != VT.getScalarType())
  ...
  for (unsigned I = 0; I != NumVectorElts; I++) {
    ...
    for (SDValue Op : Ops) {
      ...
      if (Op.getOpcode() != ISD::BUILD_VECTOR && Op.getOpcode() != ISD::SPLAT_VECTOR) {
        if (Op.isUndef()) {
      ...
      SDValue ScalarOp = Op.getOperand(Op.getOpcode() == ISD::SPLAT_VECTOR ? 0 : I);
      ...
      if (ScalarVT.isInteger() && ScalarVT.bitsGT(InSVT)) {
        ...
        // legalization.
        if (NewNodesMustHaveLegalTypes && !ScalarOp.isUndef() && !isa<ConstantSDNode>(ScalarOp)
            && TLI->getTypeAction(*getContext(), InSVT) != TargetLowering::TypeLegal) {
          return SDValue();
        }
    ...
    if (!ScalarResult.isUndef() && ScalarResult.getOpcode() != ISD::Constant && ScalarResult.getOpcode() != ISD::ConstantFP) {
      return SDValue();
    }
  ...
  SDValue V = NumElts.isScalable() ? getSplatVector(VT, DL, ScalarResults[0]) : getBuildVector(VT, DL, ScalarResults);
  NewSDValueDbgMsg(V, "New node fold constant vector: ", this);
  return V;
}

SDValue SelectionDAG::foldConstantFPMath(unsigned Opcode, const SDLoc& DL, EVT VT, ArrayRef<SDValue> Ops) {
  ...
  if (N1CFP && N2CFP) {
    ...
    switch (Opcode) {
      case ISD::FADD: C1.add(C2, APFloat::rmNearestTiesToEven); return getConstantFP(C1, DL, VT);
      case ISD::FSUB: C1.subtract(C2, APFloat::rmNearestTiesToEven); return getConstantFP(C1, DL, VT);
      case ISD::FMUL: C1.multiply(C2, APFloat::rmNearestTiesToEven); return getConstantFP(C1, DL, VT);
      case ISD::FDIV: C1.divide(C2, APFloat::rmNearestTiesToEven); return getConstantFP(C1, DL, VT);
      case ISD::FREM: C1.mod(C2); return getConstantFP(C1, DL, VT);
      case ISD::FCOPYSIGN: C1.copySign(C2); return getConstantFP(C1, DL, VT);
      case ISD::FMINNUM: return getConstantFP(minnum(C1, C2), DL, VT);
      case ISD::FMAXNUM: return getConstantFP(maxnum(C1, C2), DL, VT);
      case ISD::FMINIMUM: return getConstantFP(minimum(C1, C2), DL, VT);
      case ISD::FMAXIMUM: return getConstantFP(maximum(C1, C2), DL, VT);
      case ISD::FMINIMUMNUM: return getConstantFP(minimumnum(C1, C2), DL, VT);
      case ISD::FMAXIMUMNUM: return getConstantFP(maximumnum(C1, C2), DL, VT);
      default: break;
    }
  }
  if (N1CFP && Opcode == ISD::FP_ROUND) {
    ...
    // This can return overflow, underflow, or inexact; we don't care.
    // FIXME need to be more flexible about rounding mode.
    (void)C1.convert(VT.getFltSemantics(), APFloat::rmNearestTiesToEven, &Unused);
...
SDValue SelectionDAG::getAssertAlign(const SDLoc& DL, SDValue Val, Align A) {
  ...
  auto* N = newSDNode<AssertAlignSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, A);
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, SDValue N2) {
  SDNodeFlags Flags;
  if (Inserter) {
    Flags = Inserter->getFlags();
  }
...
void SelectionDAG::canonicalizeCommutativeBinop(unsigned Opcode, SDValue& N1, SDValue& N2) const {
  if (!TLI->isCommutativeBinOp(Opcode)) {
    return;
  }
  ...
  else if (N1.getOpcode() == ISD::SPLAT_VECTOR && N2.getOpcode() == ISD::STEP_VECTOR) {
    std::swap(N1, N2);
  }
}

SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, SDValue N2, const SDNodeFlags Flags) {
  assert(N1.getOpcode() != ISD::DELETED_NODE && N2.getOpcode() != ISD::DELETED_NODE && "Operand is DELETED_NODE!");
  ...
  // to zero etc.
  ConstantSDNode* N2CV = isConstOrConstSplat(N2, /*AllowUndefs*/ false, /*AllowTruncation*/ true);

  switch (Opcode) {
    default: break;
    case ISD::TokenFactor:
      assert(VT == MVT::Other && N1.getValueType() == MVT::Other && N2.getValueType() == MVT::Other && "Invalid token factor!");
    ...
    case ISD::AND:
      ...
      assert(N1.getValueType() == N2.getValueType() && N1.getValueType() == VT && "Binary operator types must match!");
    ...
    case ISD::SUB:
      ...
      if ((Opcode == ISD::ADD || Opcode == ISD::SUB) && VT.getScalarType() == MVT::i1) {
    ...
    case ISD::SCMP:
    case ISD::UCMP:
      assert(N1.getValueType() == N2.getValueType() && "Types of operands of UCMP/SCMP must match");
      assert(N1.getValueType().isVector() == VT.isVector() && "Operands and return type of must both be scalars or vectors");
      if (VT.isVector()) {
        assert(VT.getVectorElementCount() == N1.getValueType().getVectorElementCount()
    ...
    case ISD::FCOPYSIGN:  // N1 and result must match.  N1/N2 need not match.
      assert(N1.getValueType() == VT && N1.getValueType().isFloatingPoint() && N2.getValueType().isFloatingPoint()
             && "Invalid FCOPYSIGN!");
      break;
    case ISD::SHL:
      if (N2C && (N1.getOpcode() == ISD::VSCALE) && Flags.hasNoSignedWrap()) {
    ...
    case ISD::ROTL:
    case ISD::ROTR:
      assert(VT == N1.getValueType() && "Shift operators return type must be the same as their first arg");
      assert(VT.isInteger() && N2.getValueType().isInteger() && "Shifts only work on integers");
      assert((!VT.isVector() || VT == N2.getValueType()) && "Vector shift amounts must be in the same as their first arg");
      // Verify that the shift amount VT is big enough to hold valid shift
      // amounts.  This catches things like trying to shift an i1024 value by an
      // i8, which is easy to fall into in generic code that uses
      // TLI.getShiftAmount().
      assert(N2.getValueType().getScalarSizeInBits() >= Log2_32_Ceil(VT.getScalarSizeInBits())
      ...
      break;
    case ISD::FP_ROUND:
      assert(VT.isFloatingPoint() && N1.getValueType().isFloatingPoint() && VT.bitsLE(N1.getValueType()) && N2C
             && (N2C->getZExtValue() == 0 || N2C->getZExtValue() == 1) && N2.getOpcode() == ISD::TargetConstant
             && "Invalid FP_ROUND!");
      if (N1.getValueType() == VT) {
        return N1;  // noop conversion.
      }
    ...
    case ISD::AssertNoFPClass: {
      ...
      assert(llvm::to_underlying(NoFPClass) <= BitmaskEnumDetail::Mask<FPClassTest>() && "FPClassTest value too large");
      (void)NoFPClass;
      break;
    }
    case ISD::AssertSext:
    case ISD::AssertZext: {
      ...
      assert(VT.isInteger() && EVT.isInteger() && "Cannot *_EXTEND_INREG FP types");
      assert(!EVT.isVector() &&
    ...
    case ISD::SIGN_EXTEND_INREG: {
      ...
      assert(EVT.isVector() == VT.isVector() &&
           ...
           "type is vector!");
      assert((!EVT.isVector() || EVT.getVectorElementCount() == VT.getVectorElementCount())
    ...
    case ISD::FP_TO_SINT_SAT:
    case ISD::FP_TO_UINT_SAT: {
      assert(VT.isInteger() && cast<VTSDNode>(N2)->getVT().isInteger() && N1.getValueType().isFloatingPoint()
             && "Invalid FP_TO_*INT_SAT");
      assert(N1.getValueType().isVector() == VT.isVector() &&
           "FP_TO_*INT_SAT type should be vector iff the operand type is "
           "vector!");
      assert((!VT.isVector() || VT.getVectorElementCount() == N1.getValueType().getVectorElementCount())
             && "Vector element counts must match in FP_TO_*INT_SAT");
      assert(!cast<VTSDNode>(N2)->getVT().isVector() && "Type to saturate to must be a scalar.");
      assert(cast<VTSDNode>(N2)->getVT().bitsLE(VT.getScalarType()) && "Not extending!");
      break;
    }
    case ISD::EXTRACT_VECTOR_ELT:
      ...
      if (N2C && N1.getValueType().isFixedLengthVector() && N2C->getAPIntValue().uge(N1.getValueType().getVectorNumElements())) {
        return getUNDEF(VT);
      }
      ...
      // elements.
      if (N2C && N1.getOpcode() == ISD::CONCAT_VECTORS && N1.getOperand(0).getValueType().isFixedLengthVector()) {
        unsigned Factor = N1.getOperand(0).getValueType().getVectorNumElements();
        return getExtractVectorElt(DL, VT, N1.getOperand(N2C->getZExtValue() / Factor), N2C->getZExtValue() % Factor);
      }

      // EXTRACT_VECTOR_ELT of BUILD_VECTOR or SPLAT_VECTOR is often formed while
      // lowering is expanding large vector constants.
      if (N2C && (N1.getOpcode() == ISD::BUILD_VECTOR || N1.getOpcode() == ISD::SPLAT_VECTOR)) {
        assert((N1.getOpcode() != ISD::BUILD_VECTOR || N1.getValueType().isFixedLengthVector())
               && "BUILD_VECTOR used for scalable vectors");
        unsigned Index = N1.getOpcode() == ISD::BUILD_VECTOR ? N2C->getZExtValue() : 0;
      ...
      if (N1.getOpcode() == ISD::EXTRACT_SUBVECTOR && N1.getValueType().isFixedLengthVector()
          && N1.getValueType().getVectorNumElements() == 1) {
        return getNode(ISD::EXTRACT_VECTOR_ELT, DL, VT, N1.getOperand(0), N1.getOperand(1));
      }
      break;
    case ISD::EXTRACT_ELEMENT:
      assert(N2C && (unsigned)N2C->getZExtValue() < 2 && "Bad EXTRACT_ELEMENT!");
      assert(!N1.getValueType().isVector() && !VT.isVector() && (N1.getValueType().isInteger() == VT.isInteger())
    ...
    case ISD::EXTRACT_SUBVECTOR: {
      ...
      assert(VT.isVector() && N1VT.isVector() && "Extract subvector VTs must be vectors!");
      assert(VT.getVectorElementType() == N1VT.getVectorElementType() && "Extract subvector VTs must have the same element type!");
      assert((VT.isFixedLengthVector() || N1VT.isScalableVector())
             && "Cannot extract a scalable vector from a fixed length vector!");
      assert((VT.isScalableVector() != N1VT.isScalableVector() || VT.getVectorMinNumElements() <= N1VT.getVectorMinNumElements())
             && "Extract subvector must be from larger vector to smaller vector!");
      assert(N2C && "Extract subvector index must be a constant");
      assert((VT.isScalableVector() != N1VT.isScalableVector()
              || (VT.getVectorMinNumElements() + N2C->getZExtValue()) <= N1VT.getVectorMinNumElements())
             && "Extract subvector overflow!");
      assert(N2C->getAPIntValue().getBitWidth() == TLI->getVectorIdxWidth(getDataLayout())
      ...
      if (N1.getOpcode() == ISD::CONCAT_VECTORS && VT == N1.getOperand(0).getValueType()) {
      ...
      if (N1.getOpcode() == ISD::INSERT_SUBVECTOR && N2 == N1.getOperand(2) && VT == N1.getOperand(1).getValueType()) {
        return N1.getOperand(1);
      }
      break;
    }
    ...
    if (TLI->isCommutativeBinOp(Opcode)) {
      ...
      switch (Opcode) {
        ...
        case ISD::USUBSAT:
          ...
          return N1.getOpcode() == ISD::POISON ? getPOISON(VT) : getConstant(0, DL, VT);
      }
    }
  }
  ...
  if (N2.isUndef()) {
    switch (Opcode) {
      ...
      case ISD::USUBSAT:
        ...
        return N2.getOpcode() == ISD::POISON ? getPOISON(VT) : getConstant(0, DL, VT);
      case ISD::OR:
      case ISD::SADDSAT:
      case ISD::UADDSAT:
        // fold op(arg1, undef) -> an all-ones constant, fold op(arg1, poison) ->
        // poison.
        return N2.getOpcode() == ISD::POISON ? getPOISON(VT) : getAllOnesConstant(DL, VT);
    }
  }
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, SDValue N2, SDValue N3) {
  SDNodeFlags Flags;
  if (Inserter) {
    Flags = Inserter->getFlags();
  }
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, SDValue N2, SDValue N3,
  ...
  assert(N1.getOpcode() != ISD::DELETED_NODE && N2.getOpcode() != ISD::DELETED_NODE && N3.getOpcode() != ISD::DELETED_NODE
  ...
  switch (Opcode) {
    ...
    case ISD::FMAD: {
      assert(VT.isFloatingPoint() && "This operator only applies to FP types!");
      assert(N1.getValueType() == VT && N2.getValueType() == VT && N3.getValueType() == VT && "FMA types must match!");
    ...
    case ISD::SETCC: {
      ...
      assert(N1.getValueType() == N2.getValueType() && "SETCC operands must have the same type!");
      assert(VT.isVector() == N1.getValueType().isVector() && "SETCC type should be vector iff the operand type is vector!");
      assert((!VT.isVector() || VT.getVectorElementCount() == N1.getValueType().getVectorElementCount())
    ...
    case ISD::VSELECT:
      ...
      break;
    case ISD::VECTOR_SHUFFLE: llvm_unreachable("should use getVectorShuffle constructor!");
    ...
    case ISD::INSERT_VECTOR_ELT: {
      ...
      if (N3C && N1.getValueType().isFixedLengthVector() && N3C->getZExtValue() >= N1.getValueType().getVectorNumElements()) {
        return getUNDEF(VT);
      }
    ...
    case ISD::INSERT_SUBVECTOR: {
      ...
      assert(VT == N1.getValueType() && "Dest and insert subvector source types must match!");
      assert(VT.isVector() && N2VT.isVector() && "Insert subvector VTs must be vectors!");
      assert(VT.getVectorElementType() == N2VT.getVectorElementType() && "Insert subvector VTs must have the same element type!");
      assert((VT.isScalableVector() || N2VT.isFixedLengthVector())
             && "Cannot insert a scalable vector into a fixed length vector!");
      assert((VT.isScalableVector() != N2VT.isScalableVector() || VT.getVectorMinNumElements() >= N2VT.getVectorMinNumElements())
             && "Insert subvector must be from smaller vector to larger vector!");
      assert(isa<ConstantSDNode>(N3) && "Insert subvector index must be constant");
      assert((VT.isScalableVector() != N2VT.isScalableVector()
              || (N2VT.getVectorMinNumElements() + N3->getAsZExtVal()) <= VT.getVectorMinNumElements())
             && "Insert subvector overflow!");
      assert(N3->getAsAPIntVal().getBitWidth() == TLI->getVectorIdxWidth(getDataLayout())
      ...
      if (N1.isUndef() && N2.getOpcode() == ISD::EXTRACT_SUBVECTOR && N2.getOperand(1) == N3
          && N2.getOperand(0).getValueType() == VT) {
    ...
    case ISD::VECTOR_COMPRESS: {
      ...
      assert(VecVT.isVector() && MaskVT.isVector() && PassthruVT.isVector() && "All inputs must be vectors.");
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, SDValue N2, SDValue N3, SDValue N4,
                              const SDNodeFlags Flags) {
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, SDValue N1, SDValue N2, SDValue N3, SDValue N4, SDValue N5,
                              const SDNodeFlags Flags) {
...
static SDValue getMemsetValue(SDValue Value, EVT VT, SelectionDAG& DAG, const SDLoc& dl) {
  ...
  if (ConstantSDNode* C = dyn_cast<ConstantSDNode>(Value)) {
    ...
    if (VT.isInteger()) {
      bool IsOpaque = VT.getSizeInBits() > 64 || !DAG.getTargetLoweringInfo().isLegalStoreImmediate(C->getSExtValue());
  ...
  if (NumBits > 8) {
    ...
    Value       = DAG.getNode(ISD::MUL, dl, IntVT, Value, DAG.getConstant(Magic, dl, IntVT));
  }
...
/// string ptr.
static SDValue getMemsetStringVal(EVT VT, const SDLoc& dl, SelectionDAG& DAG, const TargetLowering& TLI,
  ...
  if (Slice.Array == nullptr) {
    ...
    return DAG.getNode(ISD::BITCAST, dl, VT, DAG.getConstant(0, dl, VT.changeTypeToInteger()));
  }
...
SDValue SelectionDAG::getMemBasePlusOffset(SDValue Base, TypeSize Offset, const SDLoc& DL, const SDNodeFlags Flags) {
  EVT     VT = Base.getValueType();
  SDValue Index;

  if (Offset.isScalable()) {
    Index = getVScale(DL, Base.getValueType(), APInt(Base.getValueSizeInBits().getFixedValue(), Offset.getKnownMinValue()));
  } else {
...
SDValue SelectionDAG::getMemBasePlusOffset(SDValue Ptr, SDValue Offset, const SDLoc& DL, const SDNodeFlags Flags) {
...
static bool isMemSrcFromConstant(SDValue Src, ConstantDataArraySlice& Slice) {
  ...
  } else if (Src.getOpcode() == ISD::ADD && Src.getOperand(0).getOpcode() == ISD::GlobalAddress
  ...
  return getConstantDataArrayInfo(G->getGlobal(), Slice, 8, SrcDelta + G->getOffset());
}
...
static void chainLoadsAndStoresForMemcpy(SelectionDAG& DAG, const SDLoc& dl, SmallVector<SDValue, 32>& OutChains, unsigned From,
  ...
  // Chain for all loads.
  SDValue LoadToken = DAG.getNode(ISD::TokenFactor, dl, MVT::Other, GluedLoadChains);

  for (unsigned i = From; i < To; ++i) {
    StoreSDNode* ST  = dyn_cast<StoreSDNode>(OutStoreChains[i]);
    SDValue NewStore = DAG.getTruncStore(LoadToken, dl, ST->getValue(), ST->getBasePtr(), ST->getMemoryVT(), ST->getMemOperand());
    OutChains.push_back(NewStore);
  }
}

static SDValue getMemcpyLoadsAndStores(SelectionDAG& DAG, const SDLoc& dl, SDValue Chain, SDValue Dst, SDValue Src, uint64_t Size,
                                       Align Alignment, bool isVol, bool AlwaysInline, MachinePointerInfo DstPtrInfo,
  ...
  const MemOp Op               = isZeroConstant ? MemOp::Set(Size,
                                               DstAlignCanChange,
                                               Alignment,
                                               /*IsZeroMemset*/ true,
                                               isVol)
                                                : MemOp::Copy(Size, DstAlignCanChange, Alignment, *SrcAlign, isVol, CopyFromConstant);
  if (!TLI.findOptimalMemOpLowering(MemOps,
                                    Limit,
                                    Op,
  ...
  bool         isConstant = BatchAA && SrcVal && BatchAA->pointsToConstantMemory(MemoryLocation(SrcVal, Size, AAInfo));

  MachineMemOperand::Flags MMOFlags = isVol ? MachineMemOperand::MOVolatile : MachineMemOperand::MONone;
  ...
  for (unsigned i = 0; i != NumMemOps; ++i) {
    ...
    if (CopyFromConstant && (isZeroConstant || (VT.isInteger() && !VT.isVector()))) {
    ...
    if (!Store.getNode()) {
      ...
      bool                     isDereferenceable = SrcPtrInfo.getWithOffset(SrcOff).isDereferenceable(VTSize, C, DL);
      ...
      Value = DAG.getExtLoad(ISD::EXTLOAD,
                             ...
                             VT,
                             commonAlignment(*SrcAlign, SrcOff),
                             SrcMMOFlags,
                             NewAAInfo);
      OutLoadChains.push_back(Value.getValue(1));

      Store = DAG.getTruncStore(Chain,
                                dl,
                                Value,
                                DAG.getMemBasePlusOffset(Dst, TypeSize::getFixed(DstOff), dl),
                                DstPtrInfo.getWithOffset(DstOff),
  ...
  unsigned GluedLdStLimit  = MaxLdStGlue == 0 ? TLI.getMaxGluedStoresPerMemcpy() : MaxLdStGlue;
  ...
  if (NumLdStInMemcpy) {
    ...
    if ((GluedLdStLimit <= 1) || !EnableMemCpyDAGOpt) {
      ...
      if (NumLdStInMemcpy <= GluedLdStLimit) {
        chainLoadsAndStoresForMemcpy(DAG, dl, OutChains, 0, NumLdStInMemcpy, OutLoadChains, OutStoreChains);
      } else {
        ...
        for (unsigned cnt = 0; cnt < NumberLdChain; ++cnt) {
          ...
          chainLoadsAndStoresForMemcpy(DAG, dl, OutChains, IndexFrom, IndexTo, OutLoadChains, OutStoreChains);
          GlueIter += GluedLdStLimit;
        }

        // Residual ld/st.
        if (RemainingLdStInMemcpy) {
          chainLoadsAndStoresForMemcpy(DAG, dl, OutChains, 0, RemainingLdStInMemcpy, OutLoadChains, OutStoreChains);
        }
      }
    }
  }
...
static SDValue getMemmoveLoadsAndStores(SelectionDAG& DAG, const SDLoc& dl, SDValue Chain, SDValue Dst, SDValue Src, uint64_t Size,
                                        Align Alignment, bool isVol, bool AlwaysInline, MachinePointerInfo DstPtrInfo,
...
static SDValue getMemsetStores(SelectionDAG& DAG, const SDLoc& dl, SDValue Chain, SDValue Dst, SDValue Src, uint64_t Size,
                               Align Alignment, bool isVol, bool AlwaysInline, MachinePointerInfo DstPtrInfo,
  ...
  for (unsigned i = 0; i < NumMemOps; i++) {
    ...
    if (VT.bitsLT(LargestVT)) {
      ...
      if (!LargestVT.isVector() && !VT.isVector() && TLI.isTruncateFree(LargestVT, VT)) {
        Value = DAG.getNode(ISD::TRUNCATE, dl, VT, MemSetValue);
      } else if (LargestVT.isVector() && !VT.isVector()
                 && TLI.shallExtractConstSplatVectorElementToStore(LargestVT.getTypeForEVT(*DAG.getContext()),
                                                                   VT.getSizeInBits(),
                                                                   Index)
                 && TLI.isTypeLegal(SVT) && LargestVT.getSizeInBits() == SVT.getSizeInBits()) {
...
static void checkAddrSpaceIsValidForLibcall(const TargetLowering* TLI, unsigned AS) {
  ...
  if (AS != 0 && !TLI->getTargetMachine().isNoopAddrSpaceCast(AS, 0)) {
    report_fatal_error("cannot lower memory intrinsic in address space " + Twine(AS));
  }
}

SDValue SelectionDAG::getMemcpy(SDValue Chain, const SDLoc& dl, SDValue Dst, SDValue Src, SDValue Size, Align Alignment, bool isVol,
                                bool AlwaysInline, const CallInst* CI, std::optional<bool> OverrideTailCall,
                                MachinePointerInfo DstPtrInfo, MachinePointerInfo SrcPtrInfo, const AAMDNodes& AAInfo,
  ...
  if (ConstantSize) {
    ...
    SDValue Result = getMemcpyLoadsAndStores(*this,
                                             dl,
                                             Chain,
                                             Dst,
                                             Src,
  ...
  if (TSI) {
    SDValue Result
        = TSI->EmitTargetCodeForMemcpy(*this, dl, Chain, Dst, Src, Size, Alignment, isVol, AlwaysInline, DstPtrInfo, SrcPtrInfo);
    if (Result.getNode()) {
      return Result;
    }
  }
  ...
  if (AlwaysInline) {
    ...
    return getMemcpyLoadsAndStores(*this,
                                   dl,
                                   Chain,
                                   Dst,
                                   Src,
  ...
  if (OverrideTailCall.has_value()) {
    ...
    bool LowersToMemcpy  = TLI->getLibcallName(RTLIB::MEMCPY) == StringRef("memcpy");
    bool ReturnsFirstArg = CI && funcReturnsFirstArgOfCall(*CI);
    IsTailCall           = CI && CI->isTailCall() && isInTailCallPosition(*CI, getTarget(), ReturnsFirstArg && LowersToMemcpy);
  }
  ...
  CLI.setDebugLoc(dl)
      ...
      .setLibCallee(TLI->getLibcallCallingConv(RTLIB::MEMCPY),
                    ...
                    getExternalSymbol(TLI->getLibcallName(RTLIB::MEMCPY), TLI->getPointerTy(getDataLayout())),
                    std::move(Args))
      .setDiscardResult()
...
SDValue SelectionDAG::getAtomicMemcpy(SDValue Chain, const SDLoc& dl, SDValue Dst, SDValue Src, SDValue Size, Type* SizeTy,
                                      unsigned ElemSz, bool isTailCall, MachinePointerInfo DstPtrInfo,
  ...
  RTLIB::Libcall LibraryCall = RTLIB::getMEMCPY_ELEMENT_UNORDERED_ATOMIC(ElemSz);
  ...
  CLI.setDebugLoc(dl)
      ...
      .setLibCallee(TLI->getLibcallCallingConv(LibraryCall),
                    ...
                    getExternalSymbol(TLI->getLibcallName(LibraryCall), TLI->getPointerTy(getDataLayout())),
                    std::move(Args))
      .setDiscardResult()
      .setTailCall(isTailCall);
...
SDValue SelectionDAG::getMemmove(SDValue Chain, const SDLoc& dl, SDValue Dst, SDValue Src, SDValue Size, Align Alignment,
                                 bool isVol, const CallInst* CI, std::optional<bool> OverrideTailCall,
                                 MachinePointerInfo DstPtrInfo, MachinePointerInfo SrcPtrInfo, const AAMDNodes& AAInfo,
                                 BatchAAResults* BatchAA) {
  ...
  if (ConstantSize) {
    ...
    SDValue Result = getMemmoveLoadsAndStores(*this,
                                              dl,
                                              Chain,
                                              Dst,
                                              Src,
  ...
  // code. If the target chooses to do this, this is the next best.
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemmove(*this, dl, Chain, Dst, Src, Size, Alignment, isVol, DstPtrInfo, SrcPtrInfo);
    if (Result.getNode()) {
      return Result;
    }
  }
  ...
  if (OverrideTailCall.has_value()) {
    ...
    bool LowersToMemmove = TLI->getLibcallName(RTLIB::MEMMOVE) == StringRef("memmove");
    bool ReturnsFirstArg = CI && funcReturnsFirstArgOfCall(*CI);
    IsTailCall           = CI && CI->isTailCall() && isInTailCallPosition(*CI, getTarget(), ReturnsFirstArg && LowersToMemmove);
  }
  ...
  CLI.setDebugLoc(dl)
      ...
      .setLibCallee(TLI->getLibcallCallingConv(RTLIB::MEMMOVE),
                    Dst.getValueType().getTypeForEVT(*getContext()),
                    getExternalSymbol(TLI->getLibcallName(RTLIB::MEMMOVE), TLI->getPointerTy(getDataLayout())),
                    std::move(Args))
      .setDiscardResult()
      .setTailCall(IsTailCall);
...
SDValue SelectionDAG::getAtomicMemmove(SDValue Chain, const SDLoc& dl, SDValue Dst, SDValue Src, SDValue Size, Type* SizeTy,
                                       unsigned ElemSz, bool isTailCall, MachinePointerInfo DstPtrInfo,
                                       MachinePointerInfo SrcPtrInfo) {
  // Emit a library call.
  TargetLowering::ArgListTy    Args;
  ...
  RTLIB::Libcall LibraryCall = RTLIB::getMEMMOVE_ELEMENT_UNORDERED_ATOMIC(ElemSz);
  if (LibraryCall == RTLIB::UNKNOWN_LIBCALL) {
    report_fatal_error("Unsupported element size");
  }
...
SDValue SelectionDAG::getMemset(SDValue Chain, const SDLoc& dl, SDValue Dst, SDValue Src, SDValue Size, Align Alignment, bool isVol,
                                bool AlwaysInline, const CallInst* CI, MachinePointerInfo DstPtrInfo, const AAMDNodes& AAInfo) {
  ...
  if (ConstantSize) {
    ...
    SDValue Result
        = getMemsetStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(), Alignment, isVol, false, DstPtrInfo, AAInfo);
  ...
  // code. If the target chooses to do this, this is the next best.
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemset(*this, dl, Chain, Dst, Src, Size, Alignment, isVol, AlwaysInline, DstPtrInfo);
    if (Result.getNode()) {
      return Result;
    }
  }
  ...
  if (AlwaysInline) {
    assert(ConstantSize && "AlwaysInline requires a constant size!");
    SDValue Result
        = getMemsetStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(), Alignment, isVol, true, DstPtrInfo, AAInfo);
    assert(Result && "getMemsetStores must return a valid sequence when AlwaysInline");
    return Result;
  }
  ...
  if (UseBZero) {
    ...
    CLI.setLibCallee(TLI->getLibcallCallingConv(RTLIB::BZERO),
                     Type::getVoidTy(Ctx),
                     getExternalSymbol(BzeroName, TLI->getPointerTy(DL)),
                     std::move(Args));
  } else {
    TargetLowering::ArgListTy Args;
    Args.push_back(CreateEntry(Dst, PointerType::getUnqual(Ctx)));
    ...
    CLI.setLibCallee(TLI->getLibcallCallingConv(RTLIB::MEMSET),
                     ...
                     getExternalSymbol(TLI->getLibcallName(RTLIB::MEMSET), TLI->getPointerTy(DL)),
                     std::move(Args));
  }
  bool LowersToMemset = TLI->getLibcallName(RTLIB::MEMSET) == StringRef("memset");
  ...
  bool IsTailCall      = CI && CI->isTailCall() && isInTailCallPosition(*CI, getTarget(), ReturnsFirstArg && LowersToMemset);
...
SDValue SelectionDAG::getAtomicMemset(SDValue Chain, const SDLoc& dl, SDValue Dst, SDValue Value, SDValue Size, Type* SizeTy,
                                      unsigned ElemSz, bool isTailCall, MachinePointerInfo DstPtrInfo) {
  // Emit a library call.
  TargetLowering::ArgListTy    Args;
  TargetLowering::ArgListEntry Entry;
  ...
  RTLIB::Libcall LibraryCall = RTLIB::getMEMSET_ELEMENT_UNORDERED_ATOMIC(ElemSz);
  if (LibraryCall == RTLIB::UNKNOWN_LIBCALL) {
    report_fatal_error("Unsupported element size");
  }
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc& dl, EVT MemVT, SDVTList VTList, ArrayRef<SDValue> Ops,
                                MachineMemOperand* MMO, ISD::LoadExtType ExtType) {
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, Opcode, VTList, Ops);
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<AtomicSDNode>(dl.getIROrder(), Opcode, VTList, MemVT, MMO, ExtType));
  ...
  auto* N = newSDNode<AtomicSDNode>(dl.getIROrder(), dl.getDebugLoc(), Opcode, VTList, MemVT, MMO, ExtType);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getAtomicCmpSwap(unsigned Opcode, const SDLoc& dl, EVT MemVT, SDVTList VTs, SDValue Chain, SDValue Ptr,
                                       SDValue Cmp, SDValue Swp, MachineMemOperand* MMO) {
  assert(Opcode == ISD::ATOMIC_CMP_SWAP || Opcode == ISD::ATOMIC_CMP_SWAP_WITH_SUCCESS);
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc& dl, EVT MemVT, SDValue Chain, SDValue Ptr, SDValue Val,
                                MachineMemOperand* MMO) {
  assert((Opcode == ISD::ATOMIC_LOAD_ADD || Opcode == ISD::ATOMIC_LOAD_SUB || Opcode == ISD::ATOMIC_LOAD_AND
          || Opcode == ISD::ATOMIC_LOAD_CLR || Opcode == ISD::ATOMIC_LOAD_OR || Opcode == ISD::ATOMIC_LOAD_XOR
          || Opcode == ISD::ATOMIC_LOAD_NAND || Opcode == ISD::ATOMIC_LOAD_MIN || Opcode == ISD::ATOMIC_LOAD_MAX
          || Opcode == ISD::ATOMIC_LOAD_UMIN || Opcode == ISD::ATOMIC_LOAD_UMAX || Opcode == ISD::ATOMIC_LOAD_FADD
          || Opcode == ISD::ATOMIC_LOAD_FSUB || Opcode == ISD::ATOMIC_LOAD_FMAX || Opcode == ISD::ATOMIC_LOAD_FMIN
          || Opcode == ISD::ATOMIC_LOAD_FMINIMUM || Opcode == ISD::ATOMIC_LOAD_FMAXIMUM || Opcode == ISD::ATOMIC_LOAD_UINC_WRAP
          || Opcode == ISD::ATOMIC_LOAD_UDEC_WRAP || Opcode == ISD::ATOMIC_LOAD_USUB_COND || Opcode == ISD::ATOMIC_LOAD_USUB_SAT
          || Opcode == ISD::ATOMIC_SWAP || Opcode == ISD::ATOMIC_STORE)
         && "Invalid Atomic Op");

  EVT VT = Val.getValueType();

  SDVTList VTs   = Opcode == ISD::ATOMIC_STORE ? getVTList(MVT::Other) : getVTList(VT, MVT::Other);
  SDValue  Ops[] = {Chain, Ptr, Val};
  return getAtomic(Opcode, dl, MemVT, VTs, Ops, MMO);
}

SDValue SelectionDAG::getAtomicLoad(ISD::LoadExtType ExtType, const SDLoc& dl, EVT MemVT, EVT VT, SDValue Chain, SDValue Ptr,
                                    MachineMemOperand* MMO) {
...
SDValue SelectionDAG::getMemIntrinsicNode(unsigned Opcode, const SDLoc& dl, SDVTList VTList, ArrayRef<SDValue> Ops, EVT MemVT,
                                          MachinePointerInfo PtrInfo, Align Alignment, MachineMemOperand::Flags Flags,
  ...
  MachineMemOperand* MMO = MF.getMachineMemOperand(PtrInfo, Flags, Size, Alignment, AAInfo);
...
SDValue SelectionDAG::getMemIntrinsicNode(unsigned Opcode, const SDLoc& dl, SDVTList VTList, ArrayRef<SDValue> Ops, EVT MemVT,
                                          MachineMemOperand* MMO) {
  assert((Opcode == ISD::INTRINSIC_VOID || Opcode == ISD::INTRINSIC_W_CHAIN || Opcode == ISD::PREFETCH
          || (Opcode <= (unsigned)std::numeric_limits<int>::max() && Opcode >= ISD::BUILTIN_OP_END
              && TSI->isTargetMemoryOpcode(Opcode)))
  ...
  if (VTList.VTs[VTList.NumVTs - 1] != MVT::Glue) {
    FoldingSetNodeID ID;
    AddNodeIDNode(ID, Opcode, VTList, Ops);
    ID.AddInteger(getSyntheticNodeSubclassData<MemIntrinsicSDNode>(Opcode, dl.getIROrder(), VTList, MemVT, MMO));
    ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
    ID.AddInteger(MMO->getFlags());
    ID.AddInteger(MemVT.getRawBits());
    void* IP = nullptr;
    ...
    N = newSDNode<MemIntrinsicSDNode>(Opcode, dl.getIROrder(), dl.getDebugLoc(), VTList, MemVT, MMO);
    createOperands(N, Ops);
...
SDValue SelectionDAG::getLifetimeNode(bool IsStart, const SDLoc& dl, SDValue Chain, int FrameIndex, int64_t Size, int64_t Offset) {
  const unsigned Opcode = IsStart ? ISD::LIFETIME_START : ISD::LIFETIME_END;
  const auto     VTs    = getVTList(MVT::Other);
  SDValue        Ops[2] = {Chain, getFrameIndex(FrameIndex, getTargetLoweringInfo().getFrameIndexTy(getDataLayout()), true)};
  ...
  LifetimeSDNode* N = newSDNode<LifetimeSDNode>(Opcode, dl.getIROrder(), dl.getDebugLoc(), VTs, Size, Offset);
  createOperands(N, Ops);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
  SDValue V(N, 0);
...
SDValue SelectionDAG::getPseudoProbeNode(const SDLoc& Dl, SDValue Chain, uint64_t Guid, uint64_t Index, uint32_t Attr) {
  ...
  auto* N = newSDNode<PseudoProbeSDNode>(Opcode, Dl.getIROrder(), Dl.getDebugLoc(), VTs, Guid, Index, Attr);
  createOperands(N, Ops);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
  SDValue V(N, 0);
...
static MachinePointerInfo InferPointerInfo(const MachinePointerInfo& Info, SelectionDAG& DAG, SDValue Ptr, int64_t Offset = 0) {
  ...
  if (const FrameIndexSDNode* FI = dyn_cast<FrameIndexSDNode>(Ptr)) {
    return MachinePointerInfo::getFixedStack(DAG.getMachineFunction(), FI->getIndex(), Offset);
  }

  // If this is (FI+Offset1)+Offset2, we can model it.
  if (Ptr.getOpcode() != ISD::ADD || !isa<ConstantSDNode>(Ptr.getOperand(1)) || !isa<FrameIndexSDNode>(Ptr.getOperand(0))) {
    return Info;
  }
...
SDValue SelectionDAG::getLoad(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr,
                              SDValue Offset, MachinePointerInfo PtrInfo, EVT MemVT, Align Alignment,
                              MachineMemOperand::Flags MMOFlags, const AAMDNodes& AAInfo, const MDNode* Ranges) {
  ...
  MachineFunction&   MF   = getMachineFunction();
  MachineMemOperand* MMO  = MF.getMachineMemOperand(PtrInfo, MMOFlags, Size, Alignment, AAInfo, Ranges);
...
SDValue SelectionDAG::getLoad(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr,
  ...
  if (VT == MemVT) {
    ...
    assert(VT.isInteger() == MemVT.isInteger() && "Cannot convert from FP to Int or Int -> FP!");
    assert(VT.isVector() == MemVT.isVector() && "Cannot use an ext load to convert to or from a vector!");
    assert((!VT.isVector() || VT.getVectorElementCount() == MemVT.getVectorElementCount())
           && "Cannot use an ext load to change the number of vector elements!");
  }

  assert((!MMO->getRanges()
          || (mdconst::extract<ConstantInt>(MMO->getRanges()->getOperand(0))->getBitWidth() == MemVT.getScalarSizeInBits()
              && MemVT.isInteger()))
  ...
  SDVTList         VTs   = Indexed ? getVTList(VT, Ptr.getValueType(), MVT::Other) : getVTList(VT, MVT::Other);
  SDValue          Ops[] = {Chain, Ptr, Offset};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::LOAD, VTs, Ops);
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<LoadSDNode>(dl.getIROrder(), VTs, AM, ExtType, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<LoadSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, ExtType, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getLoad(EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr, MachinePointerInfo PtrInfo, MaybeAlign Alignment,
                              MachineMemOperand::Flags MMOFlags, const AAMDNodes& AAInfo, const MDNode* Ranges) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoad(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, dl, Chain, Ptr, Undef, PtrInfo, VT, Alignment, MMOFlags, AAInfo, Ranges);
}

SDValue SelectionDAG::getLoad(EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr, MachineMemOperand* MMO) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoad(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, dl, Chain, Ptr, Undef, VT, MMO);
}

SDValue SelectionDAG::getExtLoad(ISD::LoadExtType ExtType, const SDLoc& dl, EVT VT, SDValue Chain, SDValue Ptr,
                                 MachinePointerInfo PtrInfo, EVT MemVT, MaybeAlign Alignment, MachineMemOperand::Flags MMOFlags,
                                 const AAMDNodes& AAInfo) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoad(ISD::UNINDEXED, ExtType, VT, dl, Chain, Ptr, Undef, PtrInfo, MemVT, Alignment, MMOFlags, AAInfo);
}

SDValue SelectionDAG::getExtLoad(ISD::LoadExtType ExtType, const SDLoc& dl, EVT VT, SDValue Chain, SDValue Ptr, EVT MemVT,
                                 MachineMemOperand* MMO) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoad(ISD::UNINDEXED, ExtType, VT, dl, Chain, Ptr, Undef, MemVT, MMO);
}
...
SDValue SelectionDAG::getIndexedLoad(SDValue OrigLoad, const SDLoc& dl, SDValue Base, SDValue Offset, ISD::MemIndexedMode AM) {
  ...
  auto MMOFlags = LD->getMemOperand()->getFlags() & ~(MachineMemOperand::MOInvariant | MachineMemOperand::MODereferenceable);
  return getLoad(AM,
                 LD->getExtensionType(),
...
SDValue SelectionDAG::getStore(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, MachinePointerInfo PtrInfo,
  ...
  MachineMemOperand* MMO  = MF.getMachineMemOperand(PtrInfo, MMOFlags, Size, Alignment, AAInfo);
  return getStore(Chain, dl, Val, Ptr, MMO);
}

SDValue SelectionDAG::getStore(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, MachineMemOperand* MMO) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStore(Chain, dl, Val, Ptr, Undef, Val.getValueType(), MMO, ISD::UNINDEXED);
}

SDValue SelectionDAG::getStore(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, SDValue Offset, EVT SVT,
                               MachineMemOperand* MMO, ISD::MemIndexedMode AM, bool IsTruncating) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  EVT VT = Val.getValueType();
  if (VT == SVT) {
    IsTruncating = false;
    ...
    assert(SVT.getScalarType().bitsLT(VT.getScalarType()) && "Should only be a truncating store, not extending!");
    assert(VT.isInteger() == SVT.isInteger() && "Can't do FP-INT conversion!");
    assert(VT.isVector() == SVT.isVector() && "Cannot use trunc store to convert to or from a vector!");
    assert((!VT.isVector() || VT.getVectorElementCount() == SVT.getVectorElementCount())
           && "Cannot use trunc store to change the number of vector elements!");
  }

  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed store with an offset!");
  SDVTList         VTs   = Indexed ? getVTList(Ptr.getValueType(), MVT::Other) : getVTList(MVT::Other);
  SDValue          Ops[] = {Chain, Val, Ptr, Offset};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::STORE, VTs, Ops);
  ID.AddInteger(SVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<StoreSDNode>(dl.getIROrder(), VTs, AM, IsTruncating, SVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<StoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, IsTruncating, SVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getTruncStore(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, MachinePointerInfo PtrInfo, EVT SVT,
                                    Align Alignment, MachineMemOperand::Flags MMOFlags, const AAMDNodes& AAInfo) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  ...
  MachineFunction&   MF  = getMachineFunction();
  MachineMemOperand* MMO = MF.getMachineMemOperand(PtrInfo, MMOFlags, SVT.getStoreSize(), Alignment, AAInfo);
  return getTruncStore(Chain, dl, Val, Ptr, SVT, MMO);
}

SDValue SelectionDAG::getTruncStore(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, EVT SVT, MachineMemOperand* MMO) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStore(Chain, dl, Val, Ptr, Undef, SVT, MMO, ISD::UNINDEXED, true);
}

SDValue SelectionDAG::getIndexedStore(SDValue OrigStore, const SDLoc& dl, SDValue Base, SDValue Offset, ISD::MemIndexedMode AM) {
  StoreSDNode* ST = cast<StoreSDNode>(OrigStore);
  assert(ST->getOffset().isUndef() && "Store is already a indexed store!");
  return getStore(ST->getChain(),
                  dl,
                  ST->getValue(),
                  Base,
                  Offset,
...
SDValue SelectionDAG::getLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT, const SDLoc& dl, SDValue Chain,
                                SDValue Ptr, SDValue Offset, SDValue Mask, SDValue EVL, MachinePointerInfo PtrInfo, EVT MemVT,
                                Align Alignment, MachineMemOperand::Flags MMOFlags, const AAMDNodes& AAInfo, const MDNode* Ranges,
                                bool IsExpanding) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  ...
  MachineFunction&   MF   = getMachineFunction();
  MachineMemOperand* MMO  = MF.getMachineMemOperand(PtrInfo, MMOFlags, Size, Alignment, AAInfo, Ranges);
  return getLoadVP(AM, ExtType, VT, dl, Chain, Ptr, Offset, Mask, EVL, MemVT, MMO, IsExpanding);
}

SDValue SelectionDAG::getLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT, const SDLoc& dl, SDValue Chain,
                                SDValue Ptr, SDValue Offset, SDValue Mask, SDValue EVL, EVT MemVT, MachineMemOperand* MMO,
                                bool IsExpanding) {
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed load with an offset!");
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPLoadSDNode>(dl.getIROrder(), VTs, AM, ExtType, IsExpanding, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<VPLoadSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, ExtType, IsExpanding, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getLoadVP(EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr, SDValue Mask, SDValue EVL,
                                MachinePointerInfo PtrInfo, MaybeAlign Alignment, MachineMemOperand::Flags MMOFlags,
                                const AAMDNodes& AAInfo, const MDNode* Ranges, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED,
                   ISD::NON_EXTLOAD,
                   VT,
                   dl,
                   Chain,
...
SDValue SelectionDAG::getLoadVP(EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr, SDValue Mask, SDValue EVL,
  ...
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, dl, Chain, Ptr, Undef, Mask, EVL, VT, MMO, IsExpanding);
}

SDValue SelectionDAG::getExtLoadVP(ISD::LoadExtType ExtType, const SDLoc& dl, EVT VT, SDValue Chain, SDValue Ptr, SDValue Mask,
                                   SDValue EVL, MachinePointerInfo PtrInfo, EVT MemVT, MaybeAlign Alignment,
                                   MachineMemOperand::Flags MMOFlags, const AAMDNodes& AAInfo, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED,
                   ExtType,
                   VT,
...
SDValue SelectionDAG::getExtLoadVP(ISD::LoadExtType ExtType, const SDLoc& dl, EVT VT, SDValue Chain, SDValue Ptr, SDValue Mask,
  ...
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED, ExtType, VT, dl, Chain, Ptr, Undef, Mask, EVL, MemVT, MMO, IsExpanding);
}

SDValue SelectionDAG::getIndexedLoadVP(SDValue OrigLoad, const SDLoc& dl, SDValue Base, SDValue Offset, ISD::MemIndexedMode AM) {
...
SDValue SelectionDAG::getStoreVP(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, SDValue Offset, SDValue Mask,
                                 SDValue EVL, EVT MemVT, MachineMemOperand* MMO, ISD::MemIndexedMode AM, bool IsTruncating,
                                 bool IsCompressing) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  bool Indexed = AM != ISD::UNINDEXED;
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPStoreSDNode>(dl.getIROrder(), VTs, AM, IsTruncating, IsCompressing, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<VPStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, IsTruncating, IsCompressing, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getTruncStoreVP(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, SDValue Mask, SDValue EVL,
  ...
  MachineFunction&   MF  = getMachineFunction();
  MachineMemOperand* MMO = MF.getMachineMemOperand(PtrInfo, MMOFlags, SVT.getStoreSize(), Alignment, AAInfo);
  return getTruncStoreVP(Chain, dl, Val, Ptr, Mask, EVL, SVT, MMO, IsCompressing);
}

SDValue SelectionDAG::getTruncStoreVP(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Ptr, SDValue Mask, SDValue EVL, EVT SVT,
                                      MachineMemOperand* MMO, bool IsCompressing) {
  EVT VT = Val.getValueType();

  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  if (VT == SVT) {
    return getStoreVP(Chain,
                      dl,
                      Val,
                      Ptr,
                      getUNDEF(Ptr.getValueType()),
                      Mask,
                      EVL,
                      VT,
                      MMO,
  ...
  AddNodeIDNode(ID, ISD::VP_STORE, VTs, Ops);
  ID.AddInteger(SVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPStoreSDNode>(dl.getIROrder(), VTs, ISD::UNINDEXED, true, IsCompressing, SVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<VPStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, ISD::UNINDEXED, true, IsCompressing, SVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedStoreVP(SDValue OrigStore, const SDLoc& dl, SDValue Base, SDValue Offset, ISD::MemIndexedMode AM) {
  auto* ST = cast<VPStoreSDNode>(OrigStore);
  assert(ST->getOffset().isUndef() && "Store is already an indexed store!");
  SDVTList         VTs   = getVTList(Base.getValueType(), MVT::Other);
  SDValue          Ops[] = {ST->getChain(), ST->getValue(), Base, Offset, ST->getMask(), ST->getVectorLength()};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::VP_STORE, VTs, Ops);
  ID.AddInteger(ST->getMemoryVT().getRawBits());
  ID.AddInteger(ST->getRawSubclassData());
  ...
  auto* N = newSDNode<VPStoreSDNode>(dl.getIROrder(),
                                     dl.getDebugLoc(),
                                     VTs,
                                     AM,
                                     ST->isTruncatingStore(),
...
SDValue SelectionDAG::getStridedLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT, const SDLoc& DL, SDValue Chain,
                                       SDValue Ptr, SDValue Offset, SDValue Stride, SDValue Mask, SDValue EVL, EVT MemVT,
                                       MachineMemOperand* MMO, bool IsExpanding) {
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed load with an offset!");
  ...
  ID.AddInteger(getSyntheticNodeSubclassData<VPStridedLoadSDNode>(DL.getIROrder(), VTs, AM, ExtType, IsExpanding, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ...
  auto* N = newSDNode<VPStridedLoadSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, AM, ExtType, IsExpanding, MemVT, MMO);
  createOperands(N, Ops);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
  SDValue V(N, 0);
...
SDValue SelectionDAG::getStridedLoadVP(EVT VT, const SDLoc& DL, SDValue Chain, SDValue Ptr, SDValue Stride, SDValue Mask,
                                       SDValue EVL, MachineMemOperand* MMO, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStridedLoadVP(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, DL, Chain, Ptr, Undef, Stride, Mask, EVL, VT, MMO, IsExpanding);
}

SDValue SelectionDAG::getExtStridedLoadVP(ISD::LoadExtType ExtType, const SDLoc& DL, EVT VT, SDValue Chain, SDValue Ptr,
                                          SDValue Stride, SDValue Mask, SDValue EVL, EVT MemVT, MachineMemOperand* MMO,
                                          bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStridedLoadVP(ISD::UNINDEXED, ExtType, VT, DL, Chain, Ptr, Undef, Stride, Mask, EVL, MemVT, MMO, IsExpanding);
}

SDValue SelectionDAG::getStridedStoreVP(SDValue Chain, const SDLoc& DL, SDValue Val, SDValue Ptr, SDValue Offset, SDValue Stride,
                                        SDValue Mask, SDValue EVL, EVT MemVT, MachineMemOperand* MMO, ISD::MemIndexedMode AM,
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(
      getSyntheticNodeSubclassData<VPStridedStoreSDNode>(DL.getIROrder(), VTs, AM, IsTruncating, IsCompressing, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, DL, IP)) {
  ...
  auto* N = newSDNode<VPStridedStoreSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, AM, IsTruncating, IsCompressing, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getTruncStridedStoreVP(SDValue Chain, const SDLoc& DL, SDValue Val, SDValue Ptr, SDValue Stride, SDValue Mask,
                                             SDValue EVL, EVT SVT, MachineMemOperand* MMO, bool IsCompressing) {
  EVT VT = Val.getValueType();

  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  if (VT == SVT) {
    return getStridedStoreVP(Chain,
                             DL,
                             Val,
                             Ptr,
                             getUNDEF(Ptr.getValueType()),
  ...
  ID.AddInteger(SVT.getRawBits());
  ID.AddInteger(
      getSyntheticNodeSubclassData<VPStridedStoreSDNode>(DL.getIROrder(), VTs, ISD::UNINDEXED, true, IsCompressing, SVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, DL, IP)) {
    cast<VPStridedStoreSDNode>(E)->refineAlignment(MMO);
  ...
  auto* N = newSDNode<VPStridedStoreSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, ISD::UNINDEXED, true, IsCompressing, SVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getGatherVP(SDVTList VTs, EVT VT, const SDLoc& dl, ArrayRef<SDValue> Ops, MachineMemOperand* MMO,
  ...
  ID.AddInteger(VT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPGatherSDNode>(dl.getIROrder(), VTs, VT, MMO, IndexType));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<VPGatherSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, VT, MMO, IndexType);
  createOperands(N, Ops);

  assert(N->getMask().getValueType().getVectorElementCount() == N->getValueType(0).getVectorElementCount()
  ...
  assert(ElementCount::isKnownGE(N->getIndex().getValueType().getVectorElementCount(), N->getValueType(0).getVectorElementCount())
         && "Vector width mismatch between index and data");
  assert(isa<ConstantSDNode>(N->getScale()) && N->getScale()->getAsAPIntVal().isPowerOf2()
...
SDValue SelectionDAG::getScatterVP(SDVTList VTs, EVT VT, const SDLoc& dl, ArrayRef<SDValue> Ops, MachineMemOperand* MMO,
                                   ISD::MemIndexType IndexType) {
  ...
  ID.AddInteger(VT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPScatterSDNode>(dl.getIROrder(), VTs, VT, MMO, IndexType));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<VPScatterSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, VT, MMO, IndexType);
  createOperands(N, Ops);

  assert(N->getMask().getValueType().getVectorElementCount() == N->getValue().getValueType().getVectorElementCount()
         && "Vector width mismatch between mask and data");
  assert(N->getIndex().getValueType().getVectorElementCount().isScalable()
             == N->getValue().getValueType().getVectorElementCount().isScalable()
         && "Scalable flags of index and data do not match");
  assert(ElementCount::isKnownGE(N->getIndex().getValueType().getVectorElementCount(),
...
SDValue SelectionDAG::getMaskedLoad(EVT VT, const SDLoc& dl, SDValue Chain, SDValue Base, SDValue Offset, SDValue Mask,
                                    SDValue PassThru, EVT MemVT, MachineMemOperand* MMO, ISD::MemIndexedMode AM,
                                    ISD::LoadExtType ExtTy, bool isExpanding) {
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed masked load with an offset!");
  SDVTList         VTs   = Indexed ? getVTList(VT, Base.getValueType(), MVT::Other) : getVTList(VT, MVT::Other);
  SDValue          Ops[] = {Chain, Base, Offset, Mask, PassThru};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::MLOAD, VTs, Ops);
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedLoadSDNode>(dl.getIROrder(), VTs, AM, ExtTy, isExpanding, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<MaskedLoadSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, ExtTy, isExpanding, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedMaskedLoad(SDValue OrigLoad, const SDLoc& dl, SDValue Base, SDValue Offset,
                                           ISD::MemIndexedMode AM) {
  MaskedLoadSDNode* LD = cast<MaskedLoadSDNode>(OrigLoad);
  assert(LD->getOffset().isUndef() && "Masked load is already a indexed load!");
  return getMaskedLoad(OrigLoad.getValueType(),
                       dl,
                       LD->getChain(),
                       Base,
                       Offset,
...
SDValue SelectionDAG::getMaskedStore(SDValue Chain, const SDLoc& dl, SDValue Val, SDValue Base, SDValue Offset, SDValue Mask,
                                     EVT MemVT, MachineMemOperand* MMO, ISD::MemIndexedMode AM, bool IsTruncating,
                                     bool IsCompressing) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed masked store with an offset!");
  SDVTList         VTs   = Indexed ? getVTList(Base.getValueType(), MVT::Other) : getVTList(MVT::Other);
  SDValue          Ops[] = {Chain, Val, Base, Offset, Mask};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::MSTORE, VTs, Ops);
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedStoreSDNode>(dl.getIROrder(), VTs, AM, IsTruncating, IsCompressing, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<MaskedStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, IsTruncating, IsCompressing, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedMaskedStore(SDValue OrigStore, const SDLoc& dl, SDValue Base, SDValue Offset,
                                            ISD::MemIndexedMode AM) {
  MaskedStoreSDNode* ST = cast<MaskedStoreSDNode>(OrigStore);
  assert(ST->getOffset().isUndef() && "Masked store is already a indexed store!");
  return getMaskedStore(ST->getChain(),
                        dl,
                        ST->getValue(),
                        Base,
                        Offset,
...
SDValue SelectionDAG::getMaskedGather(SDVTList VTs, EVT MemVT, const SDLoc& dl, ArrayRef<SDValue> Ops, MachineMemOperand* MMO,
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedGatherSDNode>(dl.getIROrder(), VTs, MemVT, MMO, IndexType, ExtTy));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<MaskedGatherSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO, IndexType, ExtTy);
  createOperands(N, Ops);

  assert(N->getPassThru().getValueType() == N->getValueType(0) && "Incompatible type of the PassThru value in MaskedGatherSDNode");
  assert(N->getMask().getValueType().getVectorElementCount() == N->getValueType(0).getVectorElementCount()
         && "Vector width mismatch between mask and data");
  assert(N->getIndex().getValueType().getVectorElementCount().isScalable()
             == N->getValueType(0).getVectorElementCount().isScalable()
...
SDValue SelectionDAG::getMaskedScatter(SDVTList VTs, EVT MemVT, const SDLoc& dl, ArrayRef<SDValue> Ops, MachineMemOperand* MMO,
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedScatterSDNode>(dl.getIROrder(), VTs, MemVT, MMO, IndexType, IsTrunc));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<MaskedScatterSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO, IndexType, IsTrunc);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getMaskedHistogram(SDVTList VTs, EVT MemVT, const SDLoc& dl, ArrayRef<SDValue> Ops, MachineMemOperand* MMO,
                                         ISD::MemIndexType IndexType) {
  assert(Ops.size() == 7 && "Incompatible number of operands");
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedHistogramSDNode>(dl.getIROrder(), VTs, MemVT, MMO, IndexType));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<MaskedHistogramSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO, IndexType);
  createOperands(N, Ops);

  assert(N->getMask().getValueType().getVectorElementCount() == N->getIndex().getValueType().getVectorElementCount()
         && "Vector width mismatch between mask and data");
  assert(isa<ConstantSDNode>(N->getScale()) && N->getScale()->getAsAPIntVal().isPowerOf2()
         && "Scale should be a constant power of 2");
...
SDValue SelectionDAG::getGetFPEnv(SDValue Chain, const SDLoc& dl, SDValue Ptr, EVT MemVT, MachineMemOperand* MMO) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  SDVTList         VTs   = getVTList(MVT::Other);
  SDValue          Ops[] = {Chain, Ptr};
  FoldingSetNodeID ID;
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<FPStateAccessSDNode>(ISD::GET_FPENV_MEM, dl.getIROrder(), VTs, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<FPStateAccessSDNode>(ISD::GET_FPENV_MEM, dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getSetFPEnv(SDValue Chain, const SDLoc& dl, SDValue Ptr, EVT MemVT, MachineMemOperand* MMO) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  SDVTList         VTs   = getVTList(MVT::Other);
  SDValue          Ops[] = {Chain, Ptr};
  FoldingSetNodeID ID;
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<FPStateAccessSDNode>(ISD::SET_FPENV_MEM, dl.getIROrder(), VTs, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void* IP = nullptr;
  if (SDNode* E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto* N = newSDNode<FPStateAccessSDNode>(ISD::SET_FPENV_MEM, dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::simplifyShift(SDValue X, SDValue Y) {
  ...
  auto isShiftTooBig = [X](ConstantSDNode* Val) { return !Val || Val->getAPIntValue().uge(X.getScalarValueSizeInBits()); };
...
SDValue SelectionDAG::simplifyFPBinop(unsigned Opcode, SDValue X, SDValue Y, SDNodeFlags Flags) {
  ...
  bool              HasNan = (XC && XC->getValueAPF().isNaN()) || (YC && YC->getValueAPF().isNaN());
  bool              HasInf = (XC && XC->getValueAPF().isInfinity()) || (YC && YC->getValueAPF().isInfinity());
...
SDValue SelectionDAG::getVAArg(EVT VT, const SDLoc& dl, SDValue Chain, SDValue Ptr, SDValue SV, unsigned Align) {
  SDValue Ops[] = {Chain, Ptr, SV, getTargetConstant(Align, dl, MVT::i32)};
  return getNode(ISD::VAARG, dl, getVTList(VT, MVT::Other), Ops);
}

SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, ArrayRef<SDUse> Ops) {
  switch (Ops.size()) {
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, ArrayRef<SDValue> Ops) {
  SDNodeFlags Flags;
  if (Inserter) {
    Flags = Inserter->getFlags();
  }
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, EVT VT, ArrayRef<SDValue> Ops, const SDNodeFlags Flags) {
  unsigned NumOps = Ops.size();
  switch (NumOps) {
    case 0: return getNode(Opcode, DL, VT);
  ...
  switch (Opcode) {
    ...
    case ISD::SELECT_CC:
      ...
      assert(Ops[0].getValueType() == Ops[1].getValueType() && "LHS and RHS of condition must have same type!");
      assert(Ops[2].getValueType() == Ops[3].getValueType() && "True and False arms of SelectCC must have same type!");
      assert(Ops[2].getValueType() == VT && "select_cc node must be of same type as true and false value!");
    ...
    case ISD::BR_CC:
      ...
      assert(Ops[2].getValueType() == Ops[3].getValueType() && "LHS/RHS of comparison should match types!");
      break;
    case ISD::VP_ADD:
    case ISD::VP_SUB:
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, ArrayRef<EVT> ResultTys, ArrayRef<SDValue> Ops) {
  return getNode(Opcode, DL, getVTList(ResultTys), Ops);
}

SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, SDVTList VTList, ArrayRef<SDValue> Ops) {
  SDNodeFlags Flags;
  if (Inserter) {
    Flags = Inserter->getFlags();
  }
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, SDVTList VTList, ArrayRef<SDValue> Ops, const SDNodeFlags Flags) {
  if (VTList.NumVTs == 1) {
  ...
  switch (Opcode) {
    ...
    case ISD::SSUBO:
    case ISD::USUBO: {
      assert(VTList.NumVTs == 2 && Ops.size() == 2 && "Invalid add/sub overflow op!");
      assert(VTList.VTs[0].isInteger() && VTList.VTs[1].isInteger() && Ops[0].getValueType() == Ops[1].getValueType()
             && Ops[0].getValueType() == VTList.VTs[0] && "Binary operator types must match!");
      SDValue N1 = Ops[0], N2 = Ops[1];
      canonicalizeCommutativeBinop(Opcode, N1, N2);

      // (X +- 0) -> X with zero-overflow.
      ConstantSDNode* N2CV = isConstOrConstSplat(N2,
                                                 /*AllowUndefs*/ false,
      ...
      if (VTList.VTs[0].getScalarType() == MVT::i1 && VTList.VTs[1].getScalarType() == MVT::i1) {
        ...
        if (Opcode == ISD::UADDO || Opcode == ISD::SADDO) {
          return getNode(ISD::MERGE_VALUES,
                         DL,
                         VTList,
                         {getNode(ISD::XOR, DL, VTList.VTs[0], F1, F2), getNode(ISD::AND, DL, VTList.VTs[1], F1, F2)},
                         Flags);
        }
        ...
        if (Opcode == ISD::USUBO || Opcode == ISD::SSUBO) {
          ...
          return getNode(ISD::MERGE_VALUES,
                         DL,
                         VTList,
                         {getNode(ISD::XOR, DL, VTList.VTs[0], F1, F2), getNode(ISD::AND, DL, VTList.VTs[1], NotF1, F2)},
                         Flags);
        }
      }
      break;
    ...
    case ISD::SSUBO_CARRY:
    case ISD::USUBO_CARRY:
      assert(VTList.NumVTs == 2 && Ops.size() == 3 && "Invalid add/sub overflow op!");
      assert(VTList.VTs[0].isInteger() && VTList.VTs[1].isInteger() && Ops[0].getValueType() == Ops[1].getValueType()
             && Ops[0].getValueType() == VTList.VTs[0] && Ops[2].getValueType() == VTList.VTs[1]
             && "Binary operator types must match!");
      break;
    case ISD::SMUL_LOHI:
    case ISD::UMUL_LOHI: {
      assert(VTList.NumVTs == 2 && Ops.size() == 2 && "Invalid mul lo/hi op!");
      assert(VTList.VTs[0].isInteger() && VTList.VTs[0] == VTList.VTs[1] && VTList.VTs[0] == Ops[0].getValueType()
             && VTList.VTs[0] == Ops[1].getValueType() && "Binary operator types must match!");
      // Constant fold.
    ...
    case ISD::FFREXP: {
      ...
      assert(VTList.VTs[0].isFloatingPoint() && VTList.VTs[1].isInteger() && VTList.VTs[0] == Ops[0].getValueType()
             && "frexp type mismatch");

      if (const ConstantFPSDNode* C = dyn_cast<ConstantFPSDNode>(Ops[0])) {
        int     FrexpExp;
        APFloat FrexpMant = frexp(C->getValueAPF(), FrexpExp, APFloat::rmNearestTiesToEven);
        SDValue Result0   = getConstantFP(FrexpMant, DL, VTList.VTs[0]);
        SDValue Result1   = getConstant(FrexpMant.isFinite() ? FrexpExp : 0, DL, VTList.VTs[1]);
    ...
    case ISD::STRICT_FP_EXTEND:
      assert(VTList.NumVTs == 2 && Ops.size() == 2 && "Invalid STRICT_FP_EXTEND!");
      assert(VTList.VTs[0].isFloatingPoint() && Ops[1].getValueType().isFloatingPoint() && "Invalid FP cast!");
      assert(VTList.VTs[0].isVector() == Ops[1].getValueType().isVector() &&
           "STRICT_FP_EXTEND result type should be vector iff the operand "
           "type is vector!");
      assert((!VTList.VTs[0].isVector() || VTList.VTs[0].getVectorElementCount() == Ops[1].getValueType().getVectorElementCount())
             && "Vector element count mismatch!");
      assert(Ops[1].getValueType().bitsLT(VTList.VTs[0]) && "Invalid fpext node, dst <= src!");
      break;
    case ISD::STRICT_FP_ROUND:
      ...
      assert((!VTList.VTs[0].isVector() || VTList.VTs[0].getVectorElementCount() == Ops[1].getValueType().getVectorElementCount())
             && "Vector element count mismatch!");
      assert(VTList.VTs[0].isFloatingPoint() && Ops[1].getValueType().isFloatingPoint()
             && VTList.VTs[0].bitsLT(Ops[1].getValueType()) && Ops[2].getOpcode() == ISD::TargetConstant
             && (Ops[2]->getAsZExtVal() == 0 || Ops[2]->getAsZExtVal() == 1) && "Invalid STRICT_FP_ROUND!");
      break;
#if 0
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, SDVTList VTList, SDValue N1) {
  SDValue Ops[] = {N1};
  return getNode(Opcode, DL, VTList, Ops);
}

SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, SDVTList VTList, SDValue N1, SDValue N2) {
  SDValue Ops[] = {N1, N2};
  return getNode(Opcode, DL, VTList, Ops);
}

SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, SDVTList VTList, SDValue N1, SDValue N2, SDValue N3) {
  SDValue Ops[] = {N1, N2, N3};
  return getNode(Opcode, DL, VTList, Ops);
}

SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc& DL, SDVTList VTList, SDValue N1, SDValue N2, SDValue N3, SDValue N4) {
  SDValue Ops[] = {N1, N2, N3, N4};
  return getNode(Opcode, DL, VTList, Ops);
}
...
SDNode* SelectionDAG::UpdateNodeOperands(SDNode* N, SDValue Op1, SDValue Op2, SDValue Op3) {
  SDValue Ops[] = {Op1, Op2, Op3};
  return UpdateNodeOperands(N, Ops);
}

SDNode* SelectionDAG::UpdateNodeOperands(SDNode* N, SDValue Op1, SDValue Op2, SDValue Op3, SDValue Op4) {
  SDValue Ops[] = {Op1, Op2, Op3, Op4};
  return UpdateNodeOperands(N, Ops);
}

SDNode* SelectionDAG::UpdateNodeOperands(SDNode* N, SDValue Op1, SDValue Op2, SDValue Op3, SDValue Op4, SDValue Op5) {
...
SDNode* SelectionDAG::UpdateNodeOperands(SDNode* N, ArrayRef<SDValue> Ops) {
  unsigned NumOps = Ops.size();
  assert(N->getNumOperands() == NumOps && "Update with wrong number of operands");
...
void SelectionDAG::setNodeMemRefs(MachineSDNode* N, ArrayRef<MachineMemOperand*> NewMemRefs) {
  if (NewMemRefs.empty()) {
    N->clearMemRefs();
    return;
  }
  ...
  MachineMemOperand** MemRefsBuffer = Allocator.template Allocate<MachineMemOperand*>(NewMemRefs.size());
...
SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT, SDValue Op1) {
  SDVTList VTs   = getVTList(VT);
  SDValue  Ops[] = {Op1};
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT, SDValue Op1, SDValue Op2) {
  SDVTList VTs   = getVTList(VT);
  SDValue  Ops[] = {Op1, Op2};
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT, SDValue Op1, SDValue Op2, SDValue Op3) {
  SDVTList VTs   = getVTList(VT);
  SDValue  Ops[] = {Op1, Op2, Op3};
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT);
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT1, EVT VT2, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT1, VT2);
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT1, EVT VT2) {
  SDVTList VTs = getVTList(VT1, VT2);
  return SelectNodeTo(N, MachineOpc, VTs, {});
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT1, EVT VT2, EVT VT3, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT1, VT2, VT3);
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, EVT VT1, EVT VT2, SDValue Op1, SDValue Op2) {
  SDVTList VTs   = getVTList(VT1, VT2);
  SDValue  Ops[] = {Op1, Op2};
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode* SelectionDAG::SelectNodeTo(SDNode* N, unsigned MachineOpc, SDVTList VTs, ArrayRef<SDValue> Ops) {
...
/// deleting things.
SDNode* SelectionDAG::MorphNodeTo(SDNode* N, unsigned Opc, SDVTList VTs, ArrayRef<SDValue> Ops) {
...
SDNode* SelectionDAG::mutateStrictFPToFP(SDNode* Node) {
  ...
  unsigned NewOpc;
  switch (OrigOpc) {
    default: llvm_unreachable("mutateStrictFPToFP called with unexpected opcode!");
...
/// node of the specified opcode and operands, it returns that node instead of
/// the current one.
MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT) {
  SDVTList VTs = getVTList(VT);
  return getMachineNode(Opcode, dl, VTs, {});
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT, SDValue Op1) {
  SDVTList VTs   = getVTList(VT);
  SDValue  Ops[] = {Op1};
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT, SDValue Op1, SDValue Op2) {
  SDVTList VTs   = getVTList(VT);
  SDValue  Ops[] = {Op1, Op2};
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT, SDValue Op1, SDValue Op2, SDValue Op3) {
  SDVTList VTs   = getVTList(VT);
  SDValue  Ops[] = {Op1, Op2, Op3};
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT);
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT1, EVT VT2, SDValue Op1, SDValue Op2) {
  SDVTList VTs   = getVTList(VT1, VT2);
  SDValue  Ops[] = {Op1, Op2};
  return getMachineNode(Opcode, dl, VTs, Ops);
}
...
MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT1, EVT VT2, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT1, VT2);
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT1, EVT VT2, EVT VT3, SDValue Op1, SDValue Op2) {
  SDVTList VTs   = getVTList(VT1, VT2, VT3);
  SDValue  Ops[] = {Op1, Op2};
  return getMachineNode(Opcode, dl, VTs, Ops);
}
...
MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, EVT VT1, EVT VT2, EVT VT3, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT1, VT2, VT3);
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& dl, ArrayRef<EVT> ResultTys, ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(ResultTys);
  return getMachineNode(Opcode, dl, VTs, Ops);
}

MachineSDNode* SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc& DL, SDVTList VTs, ArrayRef<SDValue> Ops) {
...
SDValue SelectionDAG::getTargetExtractSubreg(int SRIdx, const SDLoc& DL, EVT VT, SDValue Operand) {
  SDValue SRIdxVal = getTargetConstant(SRIdx, DL, MVT::i32);
  SDNode* Subreg   = getMachineNode(TargetOpcode::EXTRACT_SUBREG, DL, VT, Operand, SRIdxVal);
  return SDValue(Subreg, 0);
}

/// getTargetInsertSubreg - A convenience function for creating
/// TargetOpcode::INSERT_SUBREG nodes.
SDValue SelectionDAG::getTargetInsertSubreg(int SRIdx, const SDLoc& DL, EVT VT, SDValue Operand, SDValue Subreg) {
  SDValue SRIdxVal = getTargetConstant(SRIdx, DL, MVT::i32);
  SDNode* Result   = getMachineNode(TargetOpcode::INSERT_SUBREG, DL, VT, Operand, Subreg, SRIdxVal);
  return SDValue(Result, 0);
}

/// getNodeIfExists - Get the specified node if it's already available, or
/// else return NULL.
SDNode* SelectionDAG::getNodeIfExists(unsigned Opcode, SDVTList VTList, ArrayRef<SDValue> Ops) {
  SDNodeFlags Flags;
  if (Inserter) {
    Flags = Inserter->getFlags();
  }
...
SDNode* SelectionDAG::getNodeIfExists(unsigned Opcode, SDVTList VTList, ArrayRef<SDValue> Ops, const SDNodeFlags Flags) {
  if (VTList.VTs[VTList.NumVTs - 1] != MVT::Glue) {
    FoldingSetNodeID ID;
    AddNodeIDNode(ID, Opcode, VTList, Ops);
    void* IP = nullptr;
...
bool SelectionDAG::doesNodeExist(unsigned Opcode, SDVTList VTList, ArrayRef<SDValue> Ops) {
  if (VTList.VTs[VTList.NumVTs - 1] != MVT::Glue) {
    FoldingSetNodeID ID;
    AddNodeIDNode(ID, Opcode, VTList, Ops);
    void* IP = nullptr;
...
///
/// SDNode
SDDbgValue* SelectionDAG::getDbgValue(DIVariable* Var, DIExpression* Expr, SDNode* N, unsigned R, bool IsIndirect,
                                      const DebugLoc& DL, unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc()) SDDbgValue(DbgInfo->getAlloc(),
                                              Var,
                                              Expr,
                                              SDDbgOperand::fromNode(N, R),
                                              {},
                                              IsIndirect,
                                              DL,
                                              O,
...
/// Constant
SDDbgValue* SelectionDAG::getConstantDbgValue(DIVariable* Var, DIExpression* Expr, const Value* C, const DebugLoc& DL, unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc()) SDDbgValue(DbgInfo->getAlloc(),
                                              Var,
                                              Expr,
...
/// FrameIndex
SDDbgValue* SelectionDAG::getFrameIndexDbgValue(DIVariable* Var, DIExpression* Expr, unsigned FI, bool IsIndirect,
                                                const DebugLoc& DL, unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
...
SDDbgValue* SelectionDAG::getFrameIndexDbgValue(DIVariable* Var, DIExpression* Expr, unsigned FI, ArrayRef<SDNode*> Dependencies,
...
/// VReg
SDDbgValue* SelectionDAG::getVRegDbgValue(DIVariable* Var, DIExpression* Expr, Register VReg, bool IsIndirect, const DebugLoc& DL,
                                          unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc()) SDDbgValue(DbgInfo->getAlloc(),
                                              Var,
...
SDDbgValue* SelectionDAG::getDbgValueList(DIVariable* Var, DIExpression* Expr, ArrayRef<SDDbgOperand> Locs,
                                          ArrayRef<SDNode*> Dependencies, bool IsIndirect, const DebugLoc& DL, unsigned O,
                                          bool IsVariadic) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc()) SDDbgValue(DbgInfo->getAlloc(), Var, Expr, Locs, Dependencies, IsIndirect, DL, O, IsVariadic);
}

void SelectionDAG::transferDbgValues(SDValue From, SDValue To, unsigned OffsetInBits, unsigned SizeInBits, bool InvalidateDbg) {
  ...
  SDDbgOperand FromLocOp = SDDbgOperand::fromNode(From.getNode(), From.getResNo());
  ...
  for (SDDbgValue* Dbg : GetDbgValues(FromNode)) {
    ...
    if (SizeInBits) {
      ...
      auto Fragment = DIExpression::createFragmentExpression(Expr, OffsetInBits, SizeInBits);
      if (!Fragment) {
        continue;
      }
      Expr = *Fragment;
    ...
    SDDbgValue* Clone = getDbgValueList(Var,
                                        Expr,
                                        NewLocOps,
  ...
  for (SDDbgValue* Dbg : ClonedDVs) {
    assert(is_contained(Dbg->getSDNodes(), ToNode) && "Transferred DbgValues should depend on the new SDNode");
...
void SelectionDAG::salvageDebugInfo(SDNode& N) {
      ...
      case ISD::ADD: {
        ...
        if (!isa<ConstantSDNode>(N0)) {
          ...
          for (size_t i = 0; i < OrigLocOpsSize; ++i) {
            ...
            if (NewLocOps[i].getKind() != SDDbgOperand::SDNODE || NewLocOps[i].getSDNode() != &N) {
              continue;
            }
            ...
            if (RHSConstant) {
              ...
              const auto*              TmpDIExpr = DIExpression::convertToVariadicExpression(DIExpr);
              SmallVector<uint64_t, 3> ExprOps;
              ExprOps.push_back(dwarf::DW_OP_LLVM_arg);
              ExprOps.push_back(NewLocOps.size());
              ExprOps.push_back(dwarf::DW_OP_plus);
              SDDbgOperand RHS = SDDbgOperand::fromNode(N1.getNode(), N1.getResNo());
          ...
          bool IsVariadic = DV->isVariadic() || OrigLocOpsSize != NewLocOps.size();

          auto        AdditionalDependencies = DV->getAdditionalDependencies();
          SDDbgValue* Clone                  = getDbgValueList(DV->getVariable(),
                                              DIExpr,
                                              NewLocOps,
                                              AdditionalDependencies,
                                              DV->isIndirect(),
  ...
  for (SDDbgValue* Dbg : ClonedDVs) {
    assert((!Dbg->getSDNodes().empty()
            || llvm::any_of(Dbg->getLocationOps(), [&](const SDDbgOperand& Op) { return Op.getKind() == SDDbgOperand::FRAMEIX; }))
...
SDDbgLabel* SelectionDAG::getDbgLabel(DILabel* Label, const DebugLoc& DL, unsigned O) {
  assert(cast<DILabel>(Label)->isValidLocationForIntrinsic(DL) && "Expected inlined-at fields to agree");
...
public:
  RAUWUpdateListener(SelectionDAG& d, SDNode::use_iterator& ui, SDNode::use_iterator& ue)
...
void SelectionDAG::ReplaceAllUsesWith(SDValue FromN, SDValue To) {
  ...
  assert(From->getNumValues() == 1 && FromN.getResNo() == 0 && "Cannot replace with this method!");
...
void SelectionDAG::ReplaceAllUsesWith(SDNode* From, SDNode* To) {
  ...
  for (unsigned i = 0, e = From->getNumValues(); i != e; ++i) {
    assert((!From->hasAnyUseOfValue(i) || From->getValueType(i) == To->getValueType(i))
...
void SelectionDAG::ReplaceAllUsesOfValueWith(SDValue From, SDValue To) {
  ...
  // Iterate over just the existing users of From. See the comments in
  // the ReplaceAllUsesWith above.
  SDNode::use_iterator UI = From.getNode()->use_begin(), UE = From.getNode()->use_end();
  RAUWUpdateListener   Listener(*this, UI, UE);
  while (UI != UE) {
    SDNode* User                   = UI->getUser();
...
bool operator<(const UseMemo& L, const UseMemo& R) { return (intptr_t)L.User < (intptr_t)R.User; }
...
public:
  RAUOVWUpdateListener(SelectionDAG& d, SmallVectorImpl<UseMemo>& uses) : SelectionDAG::DAGUpdateListener(d), Uses(uses) {}
};
...
static bool gluePropagatesDivergence(const SDNode* Node) {
  switch (Node->getOpcode()) {
    ...
    case ISD::CopyToReg: return false;
    default: return true;
  }
...
bool SelectionDAG::calculateDivergence(SDNode* N) {
  if (TLI->isSDNodeAlwaysUniform(N)) {
    assert(!TLI->isSDNodeSourceOfDivergence(N, FLI, UA) && "Conflicting divergence information!");
    return false;
  }
  ...
  for (const auto& Op : N->ops()) {
    ...
    if (VT != MVT::Other && Op.getNode()->isDivergent() && (VT != MVT::Glue || gluePropagatesDivergence(Op.getNode()))) {
      return true;
    }
  }
  return false;
...
void SelectionDAG::VerifyDAGDivergence() {
  ...
  for (auto* N : TopoOrder) {
    assert(calculateDivergence(N) == N->isDivergent() && "Divergence bit inconsistency detected");
  }
}
#endif
...
void SelectionDAG::ReplaceAllUsesOfValuesWith(const SDValue* From, const SDValue* To, unsigned Num) {
  // Handle the simple, trivial case efficiently.
  if (Num == 1) {
  ...
  for (unsigned UseIndex = 0, UseIndexEnd = Uses.size(); UseIndex != UseIndexEnd;) {
...
unsigned SelectionDAG::AssignTopologicalOrder() {
  ...
  assert(AllNodes.front().getOpcode() == ISD::EntryToken && "First node in topological sort is not the entry token!");
  assert(AllNodes.front().getNodeId() == 0 && "First node in topological sort has non-zero id!");
  assert(AllNodes.front().getNumOperands() == 0 && "First node in topological sort has operands!");
  assert(AllNodes.back().getNodeId() == (int)DAGSize - 1 && "Last node in topologic sort has unexpected id!");
  assert(AllNodes.back().use_empty() && "Last node in topologic sort has users!");
...
SDValue SelectionDAG::makeEquivalentMemoryOrdering(SDValue OldChain, SDValue NewMemOpChain) {
  ...
  SDValue TokenFactor = getNode(ISD::TokenFactor, SDLoc(OldChain), MVT::Other, OldChain, NewMemOpChain);
  ReplaceAllUsesOfValueWith(OldChain, TokenFactor);
  UpdateNodeOperands(TokenFactor.getNode(), OldChain, NewMemOpChain);
  return TokenFactor;
}

SDValue SelectionDAG::makeEquivalentMemoryOrdering(LoadSDNode* OldLoad, SDValue NewMemOp) {
...
SDValue SelectionDAG::getSymbolFunctionGlobalAddress(SDValue Op, Function** OutFunction) {
...
bool llvm::isNullConstantOrUndef(SDValue V) { return V.isUndef() || isNullConstant(V); }
...
bool llvm::isNeutralConstant(unsigned Opcode, SDNodeFlags Flags, SDValue V, unsigned OperandNo) {
  // NOTE: The cases should match with IR's ConstantExpr::getBinOpIdentity().
  // TODO: Target-specific opcodes could be added.
  if (auto* ConstV = isConstOrConstSplat(V,
                                         /*AllowUndefs*/ false,
                                         /*AllowTruncation*/ true)) {
    ...
    switch (Opcode) {
      ...
      case ISD::OR:
      case ISD::XOR:
      case ISD::UMAX: return Const.isZero();
      case ISD::MUL: return Const.isOne();
      case ISD::AND:
      case ISD::UMIN: return Const.isAllOnes();
      case ISD::SMAX: return Const.isMinSignedValue();
      case ISD::SMIN: return Const.isMaxSignedValue();
      case ISD::SUB:
      case ISD::SHL:
      case ISD::SRA:
      case ISD::SRL: return OperandNo == 1 && Const.isZero();
      case ISD::UDIV:
      case ISD::SDIV: return OperandNo == 1 && Const.isOne();
    }
  } else if (auto* ConstFP = isConstOrConstSplatFP(V)) {
    switch (Opcode) {
      case ISD::FADD: return ConstFP->isZero() && (Flags.hasNoSignedZeros() || ConstFP->isNegative());
      case ISD::FSUB: return OperandNo == 1 && ConstFP->isZero() && (Flags.hasNoSignedZeros() || !ConstFP->isNegative());
      case ISD::FMUL: return ConstFP->isExactlyValue(1.0);
      case ISD::FDIV: return OperandNo == 1 && ConstFP->isExactlyValue(1.0);
      case ISD::FMINNUM:
      case ISD::FMAXNUM: {
...
bool llvm::isBitwiseNot(SDValue V, bool AllowUndefs) {
  ...
  ConstantSDNode* C       = isConstOrConstSplat(V, AllowUndefs, /*AllowTruncation*/ true);
...
ConstantSDNode* llvm::isConstOrConstSplat(SDValue N, bool AllowUndefs, bool AllowTruncation) {
  ...
  APInt DemandedElts = VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorMinNumElements()) : APInt(1, 1);
  return isConstOrConstSplat(N, DemandedElts, AllowUndefs, AllowTruncation);
}

ConstantSDNode* llvm::isConstOrConstSplat(SDValue N, const APInt& DemandedElts, bool AllowUndefs, bool AllowTruncation) {
...
ConstantFPSDNode* llvm::isConstOrConstSplatFP(SDValue N, const APInt& DemandedElts, bool AllowUndefs) {
  ...
  if (BuildVectorSDNode* BV = dyn_cast<BuildVectorSDNode>(N)) {
    BitVector         UndefElements;
    ConstantFPSDNode* CN = BV->getConstantFPSplatNode(DemandedElts, &UndefElements);
    // TODO: Look into whether we should allow UndefElements in non-DemandedElts
    if (CN && (UndefElements.none() || AllowUndefs)) {
      return CN;
    }
...
MemSDNode::MemSDNode(unsigned Opc, unsigned Order, const DebugLoc& dl, SDVTList VTs, EVT memvt, MachineMemOperand* mmo)
  ...
  assert((!MMO->getType().isValid() || TypeSize::isKnownLE(memvt.getStoreSize(), MMO->getSize().getValue())) && "Size mismatch!");
}
...
bool SDValue::isOperandOf(const SDNode* N) const { return is_contained(N->op_values(), *this); }

bool SDNode::isOperandOf(const SDNode* N) const {
  return any_of(N->op_values(), [this](SDValue Op) { return this == Op.getNode(); });
}
...
bool SDValue::reachesChainWithoutSideEffects(SDValue Dest, unsigned Depth) const {
  if (*this == Dest) {
    return true;
  }
  ...
  if (getOpcode() == ISD::TokenFactor) {
    ...
    // reaches Dest.
    return llvm::all_of((*this)->ops(), [=](SDValue Op) { return Op.reachesChainWithoutSideEffects(Dest, Depth - 1); });
  }
...
void SDNode::intersectFlagsWith(const SDNodeFlags Flags) { this->Flags &= Flags; }

SDValue SelectionDAG::matchBinOpReduction(SDNode* Extract, ISD::NodeType& BinOp, ArrayRef<ISD::NodeType> CandidateBinOps,
                                          bool AllowPartials) {
  // The pattern must end in an extract from index 0.
  if (Extract->getOpcode() != ISD::EXTRACT_VECTOR_ELT || !isNullConstant(Extract->getOperand(1))) {
    return SDValue();
  }

  // Match against one of the candidate binary ops.
  SDValue Op = Extract->getOperand(0);
  if (llvm::none_of(CandidateBinOps, [Op](ISD::NodeType BinOp) { return Op.getOpcode() == unsigned(BinOp); })) {
    return SDValue();
  }
  ...
  if (Op.getValueType().isFloatingPoint()) {
    ...
    switch (CandidateBinOp) {
      case ISD::FADD:
        ...
        break;
      default: llvm_unreachable("Unhandled FP opcode for binop reduction");
    }
  }
  ...
  while (Op.getOpcode() == CandidateBinOp) {
    ...
    SDValue  Op0     = Op.getOperand(0);
    SDValue  Op1     = Op.getOperand(1);
    if (Op0.getOpcode() != ISD::EXTRACT_SUBVECTOR || Op1.getOpcode() != ISD::EXTRACT_SUBVECTOR
    ...
    if (!(Op0.getConstantOperandAPInt(1) == 0 && Op1.getConstantOperandAPInt(1) == NumElts)
        && !(Op1.getConstantOperandAPInt(1) == 0 && Op0.getConstantOperandAPInt(1) == NumElts)) {
      break;
    }
    Op = Src;
  }
...
SDValue SelectionDAG::UnrollVectorOp(SDNode* N, unsigned ResNE) {
  ...
  assert(N->getNumValues() == 1 && "Can't unroll a vector with multiple results!");
  ...
  for (i = 0; i != NE; ++i) {
    ...
    switch (N->getOpcode()) {
      default: {
        Scalars.push_back(getNode(N->getOpcode(), dl, EltVT, Operands, N->getFlags()));
        break;
      }
      case ISD::VSELECT: Scalars.push_back(getNode(ISD::SELECT, dl, EltVT, Operands)); break;
      case ISD::SHL:
      case ISD::SRA:
      case ISD::SRL:
      case ISD::ROTL:
      case ISD::ROTR:
        Scalars.push_back(
            getNode(N->getOpcode(), dl, EltVT, Operands[0], getShiftAmountOperand(Operands[0].getValueType(), Operands[1])));
        break;
      case ISD::SIGN_EXTEND_INREG: {
        EVT ExtVT = cast<VTSDNode>(Operands[1])->getVT().getVectorElementType();
        Scalars.push_back(getNode(N->getOpcode(), dl, EltVT, Operands[0], getValueType(ExtVT)));
        break;
      }
      case ISD::ADDRSPACECAST: {
        const auto* ASC = cast<AddrSpaceCastSDNode>(N);
        Scalars.push_back(getAddrSpaceCast(dl, EltVT, Operands[0], ASC->getSrcAddressSpace(), ASC->getDestAddressSpace()));
        break;
      }
    }
  }
...
std::pair<SDValue, SDValue> SelectionDAG::UnrollVectorOverflowOp(SDNode* N, unsigned ResNE) {
  unsigned Opcode = N->getOpcode();
  assert((Opcode == ISD::UADDO || Opcode == ISD::SADDO || Opcode == ISD::USUBO || Opcode == ISD::SSUBO || Opcode == ISD::UMULO
          || Opcode == ISD::SMULO)
  ...
  for (unsigned i = 0; i < NE; ++i) {
    ...
    SDValue Ov  = getSelect(dl, OvEltVT, Res.getValue(1), getBoolConstant(true, dl, OvEltVT, ResVT), getConstant(0, dl, OvEltVT));
  ...
  return std::make_pair(getBuildVector(NewResVT, dl, ResScalars), getBuildVector(NewOvVT, dl, OvScalars));
}
...
MaybeAlign SelectionDAG::InferPtrAlign(SDValue Ptr) const {
  ...
  } else if (isBaseWithConstantOffset(Ptr) && isa<FrameIndexSDNode>(Ptr.getOperand(0))) {
    // Handle FI+Cst
...
std::pair<SDValue, SDValue> SelectionDAG::SplitScalar(const SDValue& N, const SDLoc& DL, const EVT& LoVT, const EVT& HiVT) {
  assert(!LoVT.isVector() && !HiVT.isVector() && !N.getValueType().isVector() && "Split node must be a scalar type");
  SDValue Lo = getNode(ISD::EXTRACT_ELEMENT, DL, LoVT, N, getIntPtrConstant(0, DL));
  SDValue Hi = getNode(ISD::EXTRACT_ELEMENT, DL, HiVT, N, getIntPtrConstant(1, DL));
...
std::pair<EVT, EVT> SelectionDAG::GetDependentSplitDestVTs(const EVT& VT, const EVT& EnvVT, bool* HiIsEmpty) const {
  ...
  assert(VTNumElts.isScalable() == EnvNumElts.isScalable() && "Mixing fixed width and scalable vectors when enveloping a type");
  EVT LoVT, HiVT;
...
/// low/high part.
std::pair<SDValue, SDValue> SelectionDAG::SplitVector(const SDValue& N, const SDLoc& DL, const EVT& LoVT, const EVT& HiVT) {
  assert(LoVT.isScalableVector() == HiVT.isScalableVector() &&
         ...
         "vector types");
  assert(LoVT.getVectorMinNumElements() + HiVT.getVectorMinNumElements() <= N.getValueType().getVectorMinNumElements()
  ...
  Hi = getNode(ISD::EXTRACT_SUBVECTOR, DL, HiVT, N, getVectorIdxConstant(LoVT.getVectorMinNumElements(), DL));
  return std::make_pair(Lo, Hi);
}

std::pair<SDValue, SDValue> SelectionDAG::SplitEVL(SDValue N, EVT VecVT, const SDLoc& DL) {
  // Split the vector length parameter.
  // %evl -> umin(%evl, %halfnumelts) and usubsat(%evl - %halfnumelts).
  EVT VT = N.getValueType();
  assert(VecVT.getVectorElementCount().isKnownEven() && "Expecting the mask to be an evenly-sized vector");
  unsigned HalfMinNumElts = VecVT.getVectorMinNumElements() / 2;
  SDValue  HalfNumElts    = VecVT.isFixedLengthVector() ? getConstant(HalfMinNumElts, DL, VT)
...
SDValue SelectionDAG::WidenVector(const SDValue& N, const SDLoc& DL) {
  EVT VT     = N.getValueType();
  EVT WideVT = EVT::getVectorVT(*getContext(), VT.getVectorElementType(), NextPowerOf2(VT.getVectorNumElements()));
  return getInsertSubvector(DL, getUNDEF(WideVT), N, 0);
}

void SelectionDAG::ExtractVectorElements(SDValue Op, SmallVectorImpl<SDValue>& Args, unsigned Start, unsigned Count, EVT EltVT) {
  EVT VT = Op.getValueType();
  if (Count == 0) {
...
unsigned GlobalAddressSDNode::getAddressSpace() const { return getGlobal()->getType()->getAddressSpace(); }
...
bool BuildVectorSDNode::isConstantSplat(APInt& SplatValue, APInt& SplatUndef, unsigned& SplatBitSize, bool& HasAnyUndefs,
                                        unsigned MinSplatBits, bool IsBigEndian) const {
  ...
  while (VecWidth > 8) {
    ...
    if ((HighValue & ~LowUndef) != (LowValue & ~HighUndef) || MinSplatBits > HalfSize) {
      break;
    }
...
SDValue BuildVectorSDNode::getSplatValue(const APInt& DemandedElts, BitVector* UndefElements) const {
  ...
  if (!Splatted) {
    ...
    assert(getOperand(FirstDemandedIdx).isUndef() && "Can only have a splat without a constant for all undefs.");
...
bool BuildVectorSDNode::getRepeatedSequence(const APInt& DemandedElts, SmallVectorImpl<SDValue>& Sequence,
...
bool BuildVectorSDNode::getRepeatedSequence(SmallVectorImpl<SDValue>& Sequence, BitVector* UndefElements) const {
  APInt DemandedElts = APInt::getAllOnes(getNumOperands());
  return getRepeatedSequence(DemandedElts, Sequence, UndefElements);
}

ConstantSDNode* BuildVectorSDNode::getConstantSplatNode(const APInt& DemandedElts, BitVector* UndefElements) const {
  return dyn_cast_or_null<ConstantSDNode>(getSplatValue(DemandedElts, UndefElements));
}

ConstantSDNode* BuildVectorSDNode::getConstantSplatNode(BitVector* UndefElements) const {
  return dyn_cast_or_null<ConstantSDNode>(getSplatValue(UndefElements));
}

ConstantFPSDNode* BuildVectorSDNode::getConstantFPSplatNode(const APInt& DemandedElts, BitVector* UndefElements) const {
  return dyn_cast_or_null<ConstantFPSDNode>(getSplatValue(DemandedElts, UndefElements));
}

ConstantFPSDNode* BuildVectorSDNode::getConstantFPSplatNode(BitVector* UndefElements) const {
  return dyn_cast_or_null<ConstantFPSDNode>(getSplatValue(UndefElements));
}

int32_t BuildVectorSDNode::getConstantFPSplatPow2ToLog2Int(BitVector* UndefElements, uint32_t BitWidth) const {
  if (ConstantFPSDNode* CN = dyn_cast_or_null<ConstantFPSDNode>(getSplatValue(UndefElements))) {
    bool           IsExact;
    APSInt         IntVal(BitWidth);
    const APFloat& APF = CN->getValueAPF();
    if (APF.convertToInteger(IntVal, APFloat::rmTowardZero, &IsExact) != APFloat::opOK || !IsExact) {
      return -1;
    }
...
bool BuildVectorSDNode::getConstantRawBits(bool IsLittleEndian, unsigned DstEltSizeInBits, SmallVectorImpl<APInt>& RawBitElements,
                                           BitVector& UndefElements) const {
  ...
  assert(((NumSrcOps * SrcEltSizeInBits) % DstEltSizeInBits) == 0 && "Invalid bitcast scale");

  // Extract raw src bits.
  SmallVector<APInt> SrcBitElements(NumSrcOps, APInt::getZero(SrcEltSizeInBits));
  ...
  for (unsigned I = 0; I != NumSrcOps; ++I) {
    ...
    SrcBitElements[I] = CInt ? CInt->getAPIntValue().trunc(SrcEltSizeInBits) : CFP->getValueAPF().bitcastToAPInt();
  }

  // Recast to dst width.
  recastRawBits(IsLittleEndian, DstEltSizeInBits, RawBitElements, SrcBitElements, UndefElements, SrcUndeElements);
  return true;
}

void BuildVectorSDNode::recastRawBits(bool IsLittleEndian, unsigned DstEltSizeInBits, SmallVectorImpl<APInt>& DstBitElements,
                                      ArrayRef<APInt> SrcBitElements, BitVector& DstUndefElements,
  ...
  if (SrcEltSizeInBits <= DstEltSizeInBits) {
    ...
    for (unsigned I = 0; I != NumDstOps; ++I) {
      ...
      for (unsigned J = 0; J != Scale; ++J) {
        ...
        assert(SrcBits.getBitWidth() == SrcEltSizeInBits && "Illegal constant bitwidths");
...
std::optional<std::pair<APInt, APInt>> BuildVectorSDNode::isConstantSequence() const {
  ...
  if (!isa<ConstantSDNode>(getOperand(0)) || !isa<ConstantSDNode>(getOperand(1))) {
    return std::nullopt;
  }
...
bool ShuffleVectorSDNode::isSplatMask(ArrayRef<int> Mask) {
  ...
  unsigned i, e;
  for (i = 0, e = Mask.size(); i != e && Mask[i] < 0; ++i) /* search */;
...
bool SelectionDAG::isConstantIntBuildVectorOrConstantInt(SDValue N, bool AllowOpaques) const {
  N = peekThroughBitcasts(N);
  ...
  if ((N.getOpcode() == ISD::SPLAT_VECTOR) && isa<ConstantSDNode>(N.getOperand(0))) {
    return true;
  }
  return false;
}
...
bool SelectionDAG::isConstantFPBuildVectorOrConstantFP(SDValue N) const {
  ...
  if ((N.getOpcode() == ISD::SPLAT_VECTOR) && isa<ConstantFPSDNode>(N.getOperand(0))) {
    return true;
  }
...
std::optional<bool> SelectionDAG::isBoolConstant(SDValue N, bool AllowTruncation) const {
  ...
  switch (TLI->getBooleanContents(N.getValueType())) {
    ...
    case TargetLowering::ZeroOrNegativeOneBooleanContent:
      ...
      return std::nullopt;
    case TargetLowering::UndefinedBooleanContent: return CVal[0];
  }
...
void SelectionDAG::createOperands(SDNode* Node, ArrayRef<SDValue> Vals) {
  ...
  SDUse* Ops = OperandRecycler.allocate(ArrayRecycler<SDUse>::Capacity::get(Vals.size()), OperandAllocator);
  ...
  for (unsigned I = 0; I != Vals.size(); ++I) {
    ...
    // Skip Chain. It does not carry divergence.
    if (VT != MVT::Other && (VT != MVT::Glue || gluePropagatesDivergence(Ops[I].getNode())) && Ops[I].getNode()->isDivergent()) {
      IsDivergent = true;
    }
  }
...
SDValue SelectionDAG::getTokenFactor(const SDLoc& DL, SmallVectorImpl<SDValue>& Vals) {
...
SDValue SelectionDAG::getNeutralElement(unsigned Opcode, const SDLoc& DL, EVT VT, SDNodeFlags Flags) {
  switch (Opcode) {
    default: return SDValue();
    case ISD::ADD:
    case ISD::OR:
    case ISD::XOR:
    case ISD::UMAX: return getConstant(0, DL, VT);
    case ISD::MUL: return getConstant(1, DL, VT);
    case ISD::AND:
    case ISD::UMIN: return getAllOnesConstant(DL, VT);
    case ISD::SMAX: return getConstant(APInt::getSignedMinValue(VT.getSizeInBits()), DL, VT);
    case ISD::SMIN: return getConstant(APInt::getSignedMaxValue(VT.getSizeInBits()), DL, VT);
    case ISD::FADD:
    ...
    case ISD::FMUL: return getConstantFP(1.0, DL, VT);
    case ISD::FMINNUM:
    case ISD::FMAXNUM: {
      // Neutral element for fminnum is NaN, Inf or FLT_MAX, depending on FMF.
      const fltSemantics& Semantics = VT.getFltSemantics();
    ...
    case ISD::FMAXIMUM: {
      ...
      const fltSemantics& Semantics = VT.getFltSemantics();
      APFloat             NeutralAF = !Flags.hasNoInfs() ? APFloat::getInf(Semantics) : APFloat::getLargest(Semantics);
...
SDValue SelectionDAG::makeStateFunctionCall(unsigned LibFunc, SDValue Ptr, SDValue InChain, const SDLoc& DLoc) {
  ...
  SDValue                          Callee = getExternalSymbol(TLI->getLibcallName(LC), TLI->getPointerTy(getDataLayout()));
  TargetLowering::CallLoweringInfo CLI(*this);
  CLI.setDebugLoc(DLoc).setChain(InChain).setLibCallee(TLI->getLibcallCallingConv(LC),
                                                       Type::getVoidTy(*getContext()),
                                                       Callee,
                                                       std::move(Args));
...
void SelectionDAG::copyExtraInfo(SDNode* From, SDNode* To) {
  ...
  for (int PrevDepth = 0, MaxDepth = 16; MaxDepth <= 1024; PrevDepth = MaxDepth, MaxDepth *= 2, Visited.clear()) {
...
#ifndef NDEBUG
static void checkForCyclesHelper(const SDNode* N, SmallPtrSetImpl<const SDNode*>& Visited, SmallPtrSetImpl<const SDNode*>& Checked,
...
void llvm::checkForCycles(const llvm::SDNode* N, const llvm::SelectionDAG* DAG, bool force) {
#ifndef NDEBUG
  bool check = force;
...
void llvm::checkForCycles(const llvm::SelectionDAG* DAG, bool force) { checkForCycles(DAG->getRoot().getNode(), DAG, force); }
