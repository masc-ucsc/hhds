...
void append_reference_preorder(const std::vector<std::vector<int>>& children_by_id, int node_id, std::vector<int>& result) {
  result.push_back(node_id);
  for (int child_id : children_by_id[static_cast<size_t>(node_id)]) {
    append_reference_preorder(children_by_id, child_id, result);
  }
}
...
void append_reference_siblings(const std::vector<std::vector<int>>& children_by_id, int node_id, std::vector<std::vector<int>>& result) {
  ...
  for (size_t i = 0; i < siblings.size(); ++i) {
    ...
    const int child_id = siblings[i];
    append_reference_siblings(children_by_id, child_id, result);
  }
}

TEST(TreeCorrectness, ChipTypicalTraversals) {
  ...
  append_reference_preorder(children_by_id, 0, expected_preorder);
  append_reference_postorder(children_by_id, 0, expected_postorder);
  append_reference_siblings(children_by_id, 0, expected_sibling_data);

  ASSERT_EQ(hhds_sibling_data.size(), expected_sibling_data.size());
  for (size_t i = 0; i < hhds_sibling_data.size(); ++i) {
    EXPECT_EQ(hhds_sibling_data[i], expected_sibling_data[i]) << "sibling mismatch at preorder index " << i;
  }
