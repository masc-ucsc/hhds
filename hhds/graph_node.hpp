...
class __attribute__((packed)) Graph_node {  // AKA pin or node entry
public:
  Graph_node() { clear(); }

  void clear() { entry_type = Entry_type::Node; }
  ...
  [[nodiscard]] size_t get_num_local_edges() const {
    auto total = n_sedges + ((set_link | overflow_link) ? 0 : (ledge_or_overflow_or_set ? 1 : 0));
